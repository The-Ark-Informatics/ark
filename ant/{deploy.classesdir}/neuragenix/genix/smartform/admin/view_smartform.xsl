    ...
    <table width="100%">
        <tr>           
            ...
            <xsl:if test='count($strBackButton) &gt; 0'>
                <xsl:choose>
                <!-- if the previous page is a searching page-->
                <xsl:when test="contains($strBackButton,'search_smartform')">
                    <input type="button" name="back" tabindex="4" value="{$backBtnLabel}" class="uportal-button" onclick="javascript:jumpTo('{$baseActionURL}?&amp;{$strBackButton}')" />
                </xsl:when>    
                <!-- if previuos page is from study channel-->
                <xsl:otherwise>

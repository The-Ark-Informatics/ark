...
/**
 * Copy and paste extension of the standard Wicket TabbedPanel but using a model object to provide the tabs rather than a list. This allows the
 * specific tabs to be dynamic and depend on model data rather than being hard-coded.
 * 
 * @author Jonny Wray
 */
public class DynamicTabbedPanel extends Panel
{
	...
	/**
	 * Generates the container for all tabs. The default container automatically adds the css <code>class</code> attribute based on the return value of
	 * {@link #getTabContainerCssClass()}
	 * 
	 * @param id
	 *           container id
	 * @return container
	...
	protected void onBeforeRender()
	{
		...
		else if ((getSelectedTab() == -1) || (isTabVisible(getSelectedTab()) == false))
		{
			...
			if (selected == tabModel.getObject().size())
			{
				 ...
				 * 
				 * we do not need to do anything special because the check in setSelectedTab() will replace the current tab panel with an empty one
				 */
				selected = 0;
			}

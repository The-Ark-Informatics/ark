...
CREATE TABLE `pheno_dataset_field_group` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE KEY `NAME` (`NAME`,`STUDY_ID`,`ARK_FUNCTION_ID`,`ARK_USER_ID`),
  KEY `FK_PHENO_DATASET_FIELD_GROUP_STUDY_ID` (`STUDY_ID`),
  KEY `FK_PHENO_DATASET_FIELD_GROUP_ARK_FUNCTION_ID` (`ARK_FUNCTION_ID`),
  KEY `FK_PHENO_DATASET_FIELD_GROUP_ARK_USER_ID_idx` (`ARK_USER_ID`),
  CONSTRAINT `FK_PHENO_DATASET_FIELD_GROUP_ARK_USER_ID` FOREIGN KEY (`ARK_USER_ID`) REFERENCES `study`.`ark_user` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,

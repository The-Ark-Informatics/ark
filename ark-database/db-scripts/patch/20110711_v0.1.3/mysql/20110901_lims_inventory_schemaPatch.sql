...
ALTER TABLE `inv_box` ADD CONSTRAINT `fk_inv_box_rowtype` FOREIGN KEY (`ROWNOTYPE_ID`) REFERENCES `inv_col_row_type` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
ALTER TABLE `inv_box` ADD CONSTRAINT `fk_inv_box_coltype` FOREIGN KEY (`COLNOTYPE_ID`) REFERENCES `inv_col_row_type` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
ALTER TABLE `inv_box` ADD CONSTRAINT `fk_inv_box_tray` FOREIGN KEY (`TRAY_ID`) REFERENCES `inv_tray` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
CREATE INDEX `fk_inv_box_tray_idx` USING BTREE ON `inv_box`(`TRAY_ID`);
CREATE INDEX `fk_inv_box_coltype_idx` USING BTREE ON `inv_box`(`COLNOTYPE_ID`);
CREATE INDEX `fk_inv_box_rowtype_idx` USING BTREE ON `inv_box`(`ROWNOTYPE_ID`);
...
CREATE INDEX `fk_inv_tank_tray_idx` USING BTREE ON `inv_tray`(`TANK_ID`);
...
INDEX `fk_study_inv_site_study` USING BTREE (`STUDY_ID`),
INDEX `fk_study_inv_site_inv_site` USING BTREE (`INV_SITE_ID`)
)
ENGINE=InnoDB
DEFAULT CHARACTER SET=latin1 COLLATE=latin1_swedish_ci
ROW_FORMAT=Compact
...
INSERT INTO `inv_col_row_type`(id, name) VALUES (2, 'Alphabet');
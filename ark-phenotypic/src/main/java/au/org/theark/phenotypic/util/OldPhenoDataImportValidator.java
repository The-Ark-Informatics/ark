 ...
 * @author elam
 */
public class OldPhenoDataImportValidator {
	static Logger							log							= LoggerFactory.getLogger(OldPhenoDataImportValidator.class);
	...
	public OldPhenoDataImportValidator(IArkCommonService<Void> iArkCommonService, IPhenotypicService iPhenotypicService, 
	 ...
	 * NB: A new PhenoCollection (questionnaire instance) is created per row
	 * 
			...
			if (databaseIntegrityError) {
				...
				while (csvReader.readRecord()) {
					...
					if (StringUtils.isBlank(statusName)) {
						...
						status = iPhenotypicService.getDefaultPhenoCollectionStatus();
					}
					else {
						try {
							status = iPhenotypicService.getPhenoCollectionStatusByName(statusName);
						}
						catch (HibernateException he) {
							status = null;
						}
					...
					for (col = 3; col < totalCols; col++) {
						...
						PhenoCollection.setQuestionnaire(this.questionnaire);
						fieldData.setPhenoCollection(PhenoCollection);

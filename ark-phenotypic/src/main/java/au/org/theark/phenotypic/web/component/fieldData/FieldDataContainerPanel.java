...
public class FieldDataContainerPanel extends AbstractContainerPanel<PhenoCollectionVO>
{
	...
	protected WebMarkupContainer initialiseSearchPanel()
	{
		...
		{
			Study study = iArkCommonService.getStudy(sessionStudyId);
			Long phenoCollectionId = (Long) SecurityUtils.getSubject().getSession().getAttribute(au.org.theark.phenotypic.web.Constants.SESSION_PHENO_COLLECTION_ID);
			
			if (phenoCollectionId != null)
			{
				PhenoCollection phenoCollection = phenotypicService.getPhenoCollection(phenoCollectionId);
				containerForm.getModelObject().getFieldData().setCollection(phenoCollection);
				//containerForm.getModelObject().getFieldData().getField().setStudy(study);
			}
			fieldDataCollection = phenotypicService.searchFieldData(containerForm.getModelObject().getFieldData());
		}
		*/

package au.org.theark.report.web.component.viewReport.biospecimendetails.filterform;
...
import au.org.theark.report.web.component.viewReport.biospecimendetails.BiospecimenDetailsReportDataSource;
import au.org.theark.report.web.component.viewReport.form.AbstractReportFilterForm;

public class BiospecimenDetailsFilterForm extends AbstractReportFilterForm<BiospecimenDetailsReportVO> {
	...
	public BiospecimenDetailsFilterForm(String id,
			CompoundPropertyModel<BiospecimenDetailsReportVO> model) {
		super(id, model);
		this.cpModel = model;
	}
	...
	protected void onGenerateProcess(AjaxRequestTarget target) {
		...
		try {
			...
			if (design != null) {
				design.setName(au.org.theark.report.service.Constants.LIMS_BIOSPECIMEN_DETAIL_REPORT_NAME); // set the output file name to match report title
		...
		BiospecimenDetailsReportVO biospecimenDetailReportVO = new BiospecimenDetailsReportVO();
		biospecimenDetailReportVO.setStudy(getModelObject().getStudy());
		biospecimenDetailReportVO.setSubjectUID(getModelObject().getSubjectUID());
		
		BiospecimenDetailsReportDataSource reportDS= new BiospecimenDetailsReportDataSource(reportService, biospecimenDetailReportVO);

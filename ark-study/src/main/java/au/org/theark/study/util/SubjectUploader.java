...
/**
 * SubjectUploader provides support for uploading subject matrix-formatted
 * files. It features state-machine behaviour to allow an external class to deal
 * with how to store the data pulled out of the files.
 * 
 * @author cellis
 */
public class SubjectUploader {
	 ...
	 *            common ARK service to perform select/insert/updates to the
	 *            database
	 * @param iStudyService
	 *            study service to perform select/insert/updates to the study
	 *            database
	 */
	public SubjectUploader(Study study, IArkCommonService iArkCommonService,
	...
	/**
	 * Imports the subject data file to the database tables Assumes the file is
	 * in the default "matrix" file format: SUBJECTUID,FIELD1,FIELD2,FIELDN...
	 ...
	 */
	public void uploadMatrixSubjectFile(InputStream fileInputStream,
			long inLength, String inFileFormat, char inDelimChr)
		...
		try {
			...
			while (csvReader.readRecord()) {
				...
				// Loop through columns in current row in file, starting from
				// the 2nd (i=1) position
				for (int i = 1; i < stringLineArray.length; i++) {
					// Print out column details
					log.debug(fieldNameArray[i] + "\t" + stringLineArray[i]);

					log.debug("Creating new field data for: "
							+ Constants.SUBJECTUID + ": " + subjectUID + "\t"
							+ Constants.DATE_COLLECTED + ": "
							+ stringLineArray[1] + "\tFIELD: "
							+ fieldNameArray[i] + "\tVALUE: "
							+ stringLineArray[i]);

					// Update progress
					curPos += stringLineArray[i].length() + 1; // update
																// progress

					// Debug only - Show progress and speed
					log.debug("progress: "
							+ decimalFormat.format(getProgress())
							+ " % | speed: " + decimalFormat.format(getSpeed())
							+ " KB/sec");
				}
	...
	/**
	 * Imports the subject data file to the database tables, and creates report
	 * on the process Assumes the file is in the default "matrix" file format:
	 ...
	 */
	public StringBuffer uploadAndReportMatrixSubjectFile(
		...
		try {
			...
			if (srcLength <= 0) {
				uploadReport
						.append("The input size was not greater than 0. Actual length reported: ");
			...
			while (csvReader.readRecord()) {
				...
				try {
					linkSubjectStudy = iArkCommonService
				...
				if (linkSubjectStudy.getId() == null
						&& linkSubjectStudy.getPerson().getId() == null) {
					person = new Person();
				} else {
				...
				if (csvReader.getIndex("FIRST_NAME") > 0)
					person.setFirstName(stringLineArray[csvReader
							.getIndex("FIRST_NAME")]);

				if (csvReader.getIndex("MIDDLE_NAME") > 0)
					person.setMiddleName(stringLineArray[csvReader
							.getIndex("MIDDLE_NAME")]);

				if (csvReader.getIndex("LAST_NAME") > 0)
					person.setLastName(stringLineArray[csvReader
							.getIndex("LAST_NAME")]);

				if (csvReader.getIndex("PREFERRED_NAME") > 0)
					person.setPreferredName(stringLineArray[csvReader
							.getIndex("PREFERRED_NAME")]);

				if (csvReader.getIndex("GENDER_TYPE") > 0
						|| csvReader.getIndex("GENDER") > 0
						|| csvReader.getIndex("SEX") > 0) {
					...
					if (stringLineArray[index] != null
							&& stringLineArray[index].length() > 0) {
						genderType = iArkCommonService
								.getGenderType(stringLineArray[index]);
				...
				if (csvReader.getIndex("VITAL_STATUS") > 0) {
					String vitalStatusStr = (stringLineArray[csvReader
							.getIndex("VITAL_STATUS")]);
					VitalStatus vitalStatus = iArkCommonService
							.getVitalStatus(vitalStatusStr);
					person.setVitalStatus(vitalStatus);
				}

				if (csvReader.getIndex("PREFERRED_EMAIL") > 0) {
					person.setPreferredEmail(stringLineArray[csvReader
							.getIndex("PREFERRED_EMAIL")]);
				}

				if (csvReader.getIndex("OTHER_EMAIL") > 0) {
					person.setPreferredEmail(stringLineArray[csvReader
							.getIndex("OTHER_EMAIL")]);
				}

				if (csvReader.getIndex("TITLE") > 0) {
					String titleStr = (stringLineArray[csvReader
							.getIndex("TITLE")]);
					TitleType titleType = iArkCommonService
							.getTitleType(titleStr);
					person.setTitleType(titleType);
				}

				if (csvReader.getIndex("MARITAL_STATUS") > 0) {
					String maritalStatusStr = (stringLineArray[csvReader
							.getIndex("MARITAL_STATUS")]);
					MaritalStatus maritalStatus = iArkCommonService
							.getMaritalStatus(maritalStatusStr);
					person.setMaritalStatus(maritalStatus);
				}

				if (csvReader.getIndex("PERSON_CONTACT_METHOD") > 0
						|| csvReader.getIndex("CONTACT_METHOD") > 0) {
					String personContactMethodStr = null;
					if (csvReader.getIndex("PERSON_CONTACT_METHOD") > 0) {
						personContactMethodStr = (stringLineArray[csvReader
								.getIndex("PERSON_CONTACT_METHOD")]);
					} else {
						personContactMethodStr = (stringLineArray[csvReader
								.getIndex("CONTACT_METHOD")]);
					}
					PersonContactMethod personContactMethod = iArkCommonService
							.getPersonContactMethod(personContactMethodStr);
					person.setPersonContactMethod(personContactMethod);
				}

				if (csvReader.getIndex("STATUS") > 0) {
					String statusStr = (stringLineArray[csvReader
							.getIndex("STATUS")]);
					SubjectStatus subjectStatus = iArkCommonService
							.getSubjectStatus(statusStr);
				...
				if (subjectVo.getSubjectStudy().getId() == null
						|| subjectVo.getSubjectStudy().getPerson().getId() == 0) {
					// Save new Subject
					try {
						...
						uploadReport.append("Subject UID must be unique:"
								+ subjectUID);
			...
			uploadReport
					.append("Unexpected I/O exception whilst reading the subject data file\n");
			log.error("processMatrixSubjectFile IOException stacktrace:", ioe);
			throw new ArkSystemException(
					"Unexpected I/O exception whilst reading the subject data file");
		} catch (Exception ex) {
			uploadReport
					.append("Unexpected exception whilst reading the subject data file\n");
			log.error("processMatrixSubjectFile Exception stacktrace:", ex);
			throw new ArkSystemException(
					"Unexpected exception occurred when trying to process subject data file");
		} finally {
			...
			uploadReport.append(decimalFormat
					.format(srcLength / 1024.0 / 1024.0));
	...
	public org.apache.wicket.util.file.File convertXlsToCsv(File file,
			char delimChr) {
		delimiterCharacter = delimChr;

		// File to store data in form of CSV
		org.apache.wicket.util.file.File f = new org.apache.wicket.util.file.File(
				file.getAbsoluteFile() + ".csv");
		try {

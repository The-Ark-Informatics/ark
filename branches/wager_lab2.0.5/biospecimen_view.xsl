	...
	<table width="100%">
            <tr valign="top">
                ...
                <td width="100%">
                   ...
                   <div dojoType="dojox.data.XmlStore" url="{$ajaxURL}?module=ajax"
jsId="samplestore" keyAttribute="name" label="name"  rootItem="lov" sendQuery="false">
</div>

        <input dojoType="dijit.form.FilteringSelect" store="samplestore" searchAttr="name"  id="parentType" onChange="updateSubList"/>
 		<input dojoType="dijit.form.FilteringSelect" store="samplestore" searchAttr="name"  id="childType"/>-->
 		<script>
 		function updateSubList(value) {
 			var url = '<xsl:value-of select="$ajaxURL"/>?module=ajax&amp;parentType='+value;
 			var store = new dojox.data.XmlStore({url:url, rootItem:"lov",  label:"name", keyAttribute:"name"});
 			dijit.byId("childType").store = store;

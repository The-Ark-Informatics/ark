...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="infopanel_content">
		...
		<form name="consent_form" action="{$baseActionURL}?uP_root=root&amp;action=consent&amp;PATIENT_intInternalPatientID={$PATIENT_intInternalPatientID}" method="POST">
                        ...
                        <xsl:variable name="displayPatientCommentsOnly"><xsl:value-of select="displayPatientCommentsOnly"/></xsl:variable>
                        <xsl:variable name="CONSENT_intConsentKey"><xsl:value-of select="CONSENT_intConsentKey"/></xsl:variable>
                        ...
                        <xsl:if test="$displayPatientCommentsOnly!='1'">
			<table>
				<tr>
		...
		<table width="100%">
			<tr>
				<td id="neuragenix-form-row-label-required" class="neuragenix-form-required-text"/>
				<td class="uportal-channel-table-header">Studies consented to</td>
		...
		<table border="0" width="100%" cellspacing="0">
			...
			<tr>
				...
				<td width="15%" class="uportal-label">
					<xsl:value-of select="CONSENTSTUDY_strRefDoctorDisplay"/>
				</td>
			...
			<xsl:if test="(count( study ) > 0 ) or ( string-length($strEdit) > 0)">
				...
				<form action="{$baseActionURL}?uP_root=root&amp;action=consent&amp;PATIENT_intInternalPatientID={$PATIENT_intInternalPatientID}" method="POST">
					<tr valign="top">
						...
						<td>
							<input type="text" name="CONSENTSTUDY_strRefDoctor" size="10" tabindex="48" class="uportal-input-text">
								<xsl:attribute name="value"><xsl:value-of select="CONSENTSTUDY_strRefDoctor"/></xsl:attribute>
							</input>

...
public class DefaultExecSessionFactory implements SharedSessionCapable{
	...
	private volatile ExecSessionWrapper sharedJschSession;


	private final ReentrantReadWriteLock sharedSessionLock = new ReentrantReadWriteLock();


	public DefaultExecSessionFactory() {
		this(false);
	}
	 ...
	 */
	public DefaultExecSessionFactory(boolean isSharedSession) {
	 ...
	 * @param isSharedSession true if the session is to be shared.
	 */
	public DefaultExecSessionFactory(JSch jsch, boolean isSharedSession) {
		this.jsch = jsch;
	...
	public ExecSession getSession() {
		...
		try {
			ExecSessionWrapper jschSession;
			if (this.isSharedSession) {
				...
				try {
					if (this.sharedJschSession == null || !this.sharedJschSession.isConnected()) {
						...
						try {
							if (this.sharedJschSession == null || !this.sharedJschSession.isConnected()) {
								this.sharedJschSession =  new ExecSessionWrapper( initJschSession());
							}
						}
						finally {
							this.sharedSessionLock.readLock().lock();
				...
				jschSession = new ExecSessionWrapper(initJschSession());
			}
			
			ExecSession execSession = new ExecSession(jschSession);
			execSession.connect();
	...
	private class ExecLogger implements Logger{
		private final  Log logger = LogFactory.getLog("com.jcraft.jsch");

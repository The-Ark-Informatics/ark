...
public class CassandraServiceImpl implements CassandraService {
	...
	public void dropPlinkDataCenterTables(DataCenterVo dataCenterVo) {
		StringBuffer sb = new StringBuffer("DROP TABLE ");
		sb.append(Constants.dataCenterTablePrefix(dataCenterVo) + "_PLINK_BED");
		cassandraOperations.execute(sb.toString());

		sb = new StringBuffer("DROP TABLE ");
		sb.append(Constants.dataCenterTablePrefix(dataCenterVo) + "_PLINK_FAM");
		cassandraOperations.execute(sb.toString());
		
		sb = new StringBuffer("DROP TABLE ");
		sb.append(Constants.dataCenterTablePrefix(dataCenterVo) + "_PLINK_BIM");
		cassandraOperations.execute(sb.toString());
	}
...
//		sb.append(" PRIMARY KEY (id, individualId)");
		...
		cassandraOperations.execute("CREATE INDEX IF NOT EXISTS "+ tableName + "_id ON "+ tableName +" (id);");
		cassandraOperations.execute("CREATE INDEX IF NOT EXISTS "+ tableName + "_famid ON "+ tableName +" (famid);");
		cassandraOperations.execute("CREATE INDEX IF NOT EXISTS "+ tableName + "_pheno ON "+ tableName +" (pheno);");
		cassandraOperations.execute("CREATE INDEX IF NOT EXISTS "+ tableName + "_sex ON "+ tableName +" (sex);");
...
//		        .where(eq("individualid", dataCenterVo.getIndividualId()));
		...
		if(dataCenterVo.getIndividualId() != null) {
            whereClauses.add(QueryBuilder.eq("individualId", dataCenterVo.getIndividualId() ));
        }
		
		ResultSet results = null;//cassandraOperations.getSession().execute(statement);
		...
		if(!whereClauses.isEmpty()) {
            ...
            results = cassandraOperations.getSession().execute(selectWhere);
        } else {
        	results = cassandraOperations.getSession().execute(select);
        }
	...
	public long tableFamCount(DataCenterVo dataCenterVo){
		long count=0;
		String table= Constants.dataCenterTablePrefix(dataCenterVo)+ "_PLINK_FAM";
	...
	public String[] getIndividualFamilyDetails(DataCenterVo dataCenterVo){
		...
		for ( Row row : results ) { 	
			...
			familyDetails[0]= row.getString("individualId");
			familyDetails[1]= row.getString("famId");
	...
	public String[] getIndividualFamilyDetails(DataCenterVo dataCenterVo, int id){
		String[] familyDetails=new String[6];
...
//	public int getSnpCount(DataCenterVo dataCenterVo){
//		int count =0;
//		String table= Constants.dataCenterTablePrefix(dataCenterVo)+ "_PLINK_BIM";
//		
//		Statement statement = QueryBuilder.select("id").from(table);
//		
//		
//		ResultSet results = cassandraOperations.getSession().execute(statement);
//		
...
//	}
	...
	public String[] getSnpMap(int snpId, DataCenterVo dataCenterVo){
		String snpMap[] = new String[4];
...
//		sert.value("chromosome", Integer.parseInt(bedArray[0]));
//		insert.value("snp", bedArray[1]);
//		insert.value("cm", Integer.parseInt(bedArray[2]));
//		insert.value("base_position", Integer.parseInt(bedArray[3]));
		
		for ( Row row : results ) { 	
			snpMap[0]=row.getInt("chromosome")+"";
			snpMap[1]=row.getString("snp");
			snpMap[2]=row.getInt("cm")+"";
			snpMap[3]=row.getInt("base_position")+"";
		}
		return snpMap;
	}
	
	
	public List<Long> getPersonIds(DataCenterVo dataCenterVo, QueryVo queryVo){
		...
		System.out.println("QueryVo - "+queryVo.getIndividualId()+" - "+queryVo.getFamilyId() + " - "+queryVo.getSex() + " - "+queryVo.getAffected() );
		
		if(queryVo.getIndividualId() != null) {
			System.out.println("getPersonIds individualId "+queryVo.getIndividualId());
            whereClauses.add(QueryBuilder.eq("individualId", queryVo.getIndividualId() ));
        }
		
		if(queryVo.getFamilyId() != null) {
			System.out.println("getPersonIds famId "+queryVo.getFamilyId());
            whereClauses.add(QueryBuilder.eq("famId", queryVo.getFamilyId() ));
        }
		
		if(queryVo.getSex() != null) {
			System.out.println("getPersonIds sex "+queryVo.getSex());
            whereClauses.add(QueryBuilder.eq("sex", queryVo.getSex()));
        }
		
		if(queryVo.getAffected() != null) {
			System.out.println("getPersonIds pheno "+queryVo.getAffected());
            whereClauses.add(QueryBuilder.eq("pheno", queryVo.getAffected()));
        }
		...
		for ( Row row : results ) { 
			...
			personIdList.add((long)row.getInt("id"));
		}
...
//	public String[] getPersonDetails(DataCenterVo dataCenterVo, QueryVo queryVo){
...
//	}
	...
	public int getSnpBedColumnValue(DataCenterVo dataCenterVo,long snpId ,int colId){
		int result =0;
		String table= Constants.dataCenterTablePrefix(dataCenterVo)+ "_PLINK_BED";
		Statement statement = QueryBuilder.select()
		        .all()

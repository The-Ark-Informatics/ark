...
//		}
	...
	public @ResponseBody String executeQuery(@RequestBody QueryVo query) {

		Future<Report> report = null;
		try {
			report = jobService.executeQuery(query);
		} catch (Exception e) {
			e.printStackTrace();
			report = new Future<Report>() {
	...
	@RequestMapping(value = "/dataSourceQueryResult", method = RequestMethod.POST)
	public @ResponseBody String getQueryResult(@RequestBody QueryVo query) {
		String result = "No Result Found"; 
		try {
			
			System.out.println("--------Query Result Execution-----------"+query.getIndividualId());
			
			System.out.println(query.getMicroServiceId());
			System.out.println(query.getDataSourceId());
			System.out.println(query.getDirectory());
			System.out.println(query.getSex());
			System.out.println(query.getAffected());

...
<xsl:template match="isearch">
    ...
    function submitSelectedDomain(){    
        document.frmBuildISearch.newDomainSelected.value='true'; 
        document.frmBuildISearch.submit();
    }
    ...
    <a name="frmbuild_isearch_anchor"/>
        ...
        <table width="100%">
            <tr>    
                <td width="100%">
                    ...
                    <td width="40%" class="uportal-label">
                        <select name="selected_domains" size="5" tabindex="5" class="uportal-input-text" onClick="javascript:submitSelectedDomain()">
                            ...
                            <xsl:for-each select="selected_smartform">
                            <xsl:variable name="strDomainName"><xsl:value-of select="./strDomainName" /></xsl:variable>
                                <option style="color: green; font-weight: bold">
                                    <xsl:if test="$SelectedDomainSel = $strDomainName">
                                        <xsl:attribute name="selected">true</xsl:attribute> 
                            ...
                            <xsl:for-each select="selected_study">
                            <xsl:variable name="strDomainName"><xsl:value-of select="./strDomainName" /></xsl:variable>
                                <option style="color: purple; font-weight: bold">
                                     <xsl:if test="$SelectedDomainSel = $strDomainName">
                                        <xsl:attribute name="selected">true</xsl:attribute> 
            ...
            <tr>
                    ...
                    <xsl:when test="position() mod 2 != 0">
                           ...
                           <td width="10%" class="uportal-input-text">
                            ...
                            <td width="5%" class="uportal-input-text">
                                <xsl:if test="function-available('url:encode')">
                                    <a href="#" onclick="javascript:mySubmit('&amp;delete=true&amp;fieldSel={$fieldSel}&amp;SelectedDomainSel={$domainVal}&amp;selFieldType={$selFieldType}&amp;opSel={$opSel}&amp;searchValue={url:encode($searchValue)}&amp;joinSel={$joinSel}&amp;number={$criteriaNo}&amp;labelInColumn={$labelInColumn}&amp;openBracketSel={$openBracketSel}&amp;closeBracketSel={$closeBracketSel}&amp;participantDomainSel={$participantDomainSel}')">Delete</a>
                                </xsl:if>
                            <!--/td>
                            <td width="10%"-->                                
        ...
        <input type="hidden" name="newDomainSelected" class="uportal-input-text" size="10" value=""/>
    </form>

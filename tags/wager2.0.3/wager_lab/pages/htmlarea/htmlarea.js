...
//
// $Id: htmlarea.js,v 1.1 2004/02/23 00:03:23 renny Exp $
...
HTMLArea.prototype.updateToolbar = function(noStatus) {
    ...
    if (HTMLArea.is_gecko) {
        try{
            var callwrapper = new CCallWrapper(doc, 100, 'execCommand',"useCSS",false,true);
             CCallWrapper.asyncExecute(callwrapper);
            //setTimeout(doc.execCommand("useCSS",false,true),100);
            }
        catch(e){alert(e.message);}
    }
...
HTMLArea.prototype._createLink = function(link) {
	...
	if (link) outparam = {
		f_href   : HTMLArea.setUrlBase(link.href,""),
		f_title  : link.title,
...
HTMLArea.prototype._insertImage = function(image) {
	...
	if (image) outparam = {
		f_url    : HTMLArea.setUrlBase(image.src,""),
		f_alt    : image.alt,
	...
	this._popupDialog("insert_image.html", function(param) {
		...
		if (!img) {
			var sel = editor._getSelection();
			var range = editor._createRange(sel);
			editor._doc.execCommand("insertimage", false, HTMLArea.setUrlBase(param.f_url,_base_download_url));
			if (HTMLArea.is_ie) {
			...
			img.src =HTMLArea.setUrlBase(param.f_url,_base_download_url);
		}
		for (field in param) {
...
HTMLArea.checkSupportedBrowser = function() {
	if (HTMLArea.is_gecko) {
		if (navigator.productSub < 20030210) {
			alert("This browser only allows you to edit raw HTML.\n" +
			      "For WYSIWYG editing support, please download\n"+
			      "the latest stable version of Mozilla from mozilla.org \n");
			return false;
		}
	}
		...
		if (HTMLArea.is_ie && root_tag == "head") {
			...
			for (i = 0; i < attrs.length; ++i) {
				...
				if(/^(src|href)$/.test(name)){
					value = HTMLArea.setUrlBase(value,"");
				}
				html += " " + name + '="' + value + '"';
			}
			html += closed ? " />" : ">";
...
// ASV added code to strip absolutized paths back down
HTMLArea.setUrlBase = function(url, base){
	if(url.indexOf(_base_download_url)==0){
		url=url.substring(_base_download_url.length);
	}
	if(url.indexOf(trimbase)==0){
		url = url.substring(trimbase.length);
	}
	if(url.indexOf("/")==0){
		url = base+url;
	}
	//alert("changed url to \n"+url+"\n trimbase was "+trimbase+" \ndownload was "+_base_download_url);
	return url;
}

HTMLArea.prototype.setDownloadLinks = function(){
	var ilist = this._doc.getElementsByTagName("img");
	//alert("Setting up "+ilist.length+" downloads");
	for(var i=0; i<ilist.length; i++){
		var img = ilist.item(i);
		//var atts = img.attributes;
		//if(atts){
		//    var att = atts.getNamedItem("src");
		//    if (att){
        //       att.nodeValue = HTMLArea.setUrlBase(att.nodeValue,_base_download_url)
		//    }
		//}
		//alert("changing "+val);
		var val = img.getAttribute("src");
		//alert("changed "+val+" to "+HTMLArea.setUrlBase(val,_base_download_url));
		img.setAttribute("src",HTMLArea.setUrlBase(val,_base_download_url));
	}
}
// end ASV

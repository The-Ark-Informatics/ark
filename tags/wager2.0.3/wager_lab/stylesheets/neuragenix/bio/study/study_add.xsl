...
<xsl:include href="../../common/common_btn_name.xsl"/>
  ...
  <xsl:param name="strStudyCode"><xsl:value-of select="strStudyCode" /></xsl:param>
	...
	<tr>
        <td width="1%" class="neuragenix-form-required-text">*</td>
<td width="20%" class="uportal-label">
                                                                                <xsl:value-of select="STUDY_strBarcodeSourceDisplay"/>:
                                                                        </td>
<td width="79%" align="left" class="uportal-label" id="neuragenix-form-row-input-input">
                                                                                <select name="strBarcodeSource"  class="uportal-input-text" tabindex="11">
<xsl:for-each select="STUDY_strBarcodeSource">
                                                                <option>
                                                                        <xsl:attribute name="value">
                                                                                <xsl:value-of select="."/>
                                                                        </xsl:attribute>

                                                        <xsl:variable name="valueDisplay">

...
import org.wager.barcode.*;
import neuragenix.bio.utilities.*;
import neuragenix.genix.config.*;
...
public class CPatients implements IChannel {
    ...
    private static final String DEFAULT_STUDY = PropertiesManager.getProperty("neuragenix.bio.DefaultStudy");
    ...
    private static boolean PROPERTY_HIDE_FLAGGED_PATIENT_NAMES = false;
    /**
     *
    ...
    static
    {
        ...
        try
        {
        	PROPERTY_HIDE_FLAGGED_PATIENT_NAMES = PropertiesManager.getPropertyAsBoolean("neuragenix.bio.patient.hideFlaggedPatientNames");
        }
        catch(Exception e)
        {
            System.out.println ("[CPatient] Property neuragenix.bio.patient.neuragenix.bio.patient.hideFlaggedPatientNames not present default to false.");
        }
...
//            studyTab = SessionManager.getTabOrder( authToken, "CStudy");
    ...
    private String doListFlaggedRecords(){
        ...
        String showFlagNamesXML = new String("<hideFlaggedPatientNames>").concat(Boolean.toString(PROPERTY_HIDE_FLAGGED_PATIENT_NAMES)).concat("</hideFlaggedPatientNames>");
        return "<flags>" + strXML.toString() + showFlagNamesXML + "</flags>";
            ...
            if( ((runtimeData.getParameter( "submit" ) == null ) || (searchQuery == null)) && 
                ...
                if( (runtimeData.getParameter("newSearch") != null ) || (searchQuery == null) ){
                  ...
                  //     searchQuery.setOrderBy( runtimeData.getParameter( "setOrderBy" ) );
                    ...
                    if( runtimeData.getParameter( "ADMISSIONS_strHospitalUR" ) != null && runtimeData.getParameter( "ADMISSIONS_strHospitalUR" ).trim().length() > 0 ){
                        searchQuery.setDomain( "ADMISSIONS", "ADMISSIONS_intPatientID",
                   ...
                   // if(runtimeData.getParameter( "intStudyID" ) != null && runtimeData.getParameter( "intStudyID" ).trim().length() > 0)
                    //{
                        ...
                        //This code sets up the allowable study keys
                	//Vector studykeys = authToken.getStudyList();
                   //     if (!studykeys.contains("ALL_STUDIES")) {
                        ...
                        searchQuery.setDomain( "STUDY", "CONSENTSTUDY_intStudyID",
                                "STUDY_intStudyID", "RIGHT JOIN");
                    ...
                    {
                    	  intStudyID = runtimeData.getParameter("intStudyID");
                    	 searchQuery.setWhere( "AND", 0, "PATIENT_intStudyID", "=",runtimeData.getParameter("intStudyID"), 0, DALQuery.WHERE_HAS_VALUE );
                    }
                          
                           
                           
                            searchQuery.setWhere("AND", 0, "PATIENT_intStudyID", "IN", StudyUtilities.getStudyIDSQLString(authToken.getStudyList()),0,DALQuery.WHERE_HAS_VALUE);
                       ...
                       // searchQuery.setWhere( "AND", 0, "CONSENTSTUDY_intStudyID", "IN", getStudyIDSQLString(studykeys), 0, DALQuery.WHERE_HAS_VALUE );
                        //if(runtimeData.getParameter( "intStudyID" ) != null && runtimeData.getParameter( "intStudyID" ).trim().length() > 0)
                   ...
                   // }
                    //System.err.println(searchQuery.convertSelectQueryToString());
                ...
                searchQuery.clearFields();
                searchQuery.setCountField("PATIENT_intInternalPatientID", false);
                System.out.println ("Query : " + searchQuery.convertSelectQueryToString());
                ResultSet tRS = searchQuery.executeSelect();
                tRS.next();
                int allRecords = tRS.getInt(1);
                tRS.close();
                tRS = null;
                
                searchQuery.clearCountField();
                searchQuery.setFields( formfields, null );
                tRS = searchQuery.executeSelect();
...
//               strXML.append( QueryChannel.buildFormLabelXMLFile( formfields ));
    ...
    private String getStudyIDSQLString (Vector studykeys) throws Exception{
  
        StringBuffer in_expr = new StringBuffer();
        in_expr.append("(");
        DALQuery studies = new DALQuery();
        studies.setDomain("STUDY",null,null,null);
        studies.setField("STUDY_intStudyID", null );
        ResultSet st_rs;
        int i;
        if (studykeys.size() != 0) {
        for (i=0; i < studykeys.size(); i++) {
        studies.setWhere(null, 0, "STUDY_strStudyName", "=", ((String) studykeys.get(i)).replace('_',' '), 0, DALQuery.WHERE_HAS_VALUE );
        st_rs = studies.executeSelect();
        if (st_rs.first() != false) {
        	in_expr.append(st_rs.getString("STUDY_intStudyID"));
    ...
    private String doAdmissions(){
        ...
        try{
            ...
            if( patientID != null ){
                    ...
                    else
                    {
                       ...
                       runtimeData.setParameter( "ADMISSIONS_dtSurgery",
                        ...
                        if( ((strError = QueryChannel.checkRequiredFields( formfields, runtimeData )) == null ) && 
                            ...
                            if( lock.isValid() && lock.lockWrites() ){
                            	int intStudyKey = Integer.parseInt(runtimeData.getParameter("ADMISSIONS_intStudyID"));
                            	int intSubStudyKey = Integer.parseInt(runtimeData.getParameter("ADMISSIONS_intSubStudyID"));
                                // Get the patient admission number
                                //
                                IAdmissionIDGenerator idgGenerator = IDGenerationFactory.getAdmissionIDGenerationInstance();
                                String studyCode = StudyUtilities.getStudyCode(intStudyKey,intSubStudyKey);
                                String admissionNumber;
                                if (studyCode == null || studyCode.equals("")) {
                           
                                 admissionNumber = getPatientAdmissionNumber (patientID);
                                }
                                else
                                 admissionNumber = idgGenerator.getAdmissionID(0, intStudyKey, intSubStudyKey, authToken);
                                
                                runtimeData.setParameter("ADMISSIONS_strAdmissionID", admissionNumber);
               ...
               strXML.append(StudyUtilities.getListOfSubStudiesXML(authToken,false, false, Integer.parseInt(patientID), 1, null));                
                lock.addLock( "PATIENT", patientID, LockRecord.READ_ONLY );
                strXML.append(StudyUtilities.getListOfStudiesXML(authToken,false, false, Integer.parseInt(patientID), 1, null));                
                lock.addLock( "PATIENT", patientID, LockRecord.READ_ONLY );
                formfields = null;
                formfields = DatabaseSchema.getFormFields( "cpatient_view_patient_admissions" );
    ...
    private String doConsent(){
        ...
        try {
            ...
            if( patientID != null ){
                ...
                strXML.append("<default_study>"+ SystemConfiguration.getConfigValue("DEFAULTSTUDY")+"</default_study>");
                //
                // display the list
...
//                    System.out.println(aQuery.convertSelectQueryToString());                
    ...
    private String getPatientAdmissionNumber (String patientID){
        ...
        try{
            ...
            query = new DALSecurityQuery("patient_admissions", authToken);
            query.setDomain("ADMISSIONS", null, null, null);
            query.setMaxField ("ADMISSIONS_intAdmissionkey");
            ...
            if(rsMax.next())
            {
                 intMaxKey = rsMax.getInt("MAX_ADMN_intAdmissionkey");                
            }
            ...
            if (intMaxKey > 0)
            {
                query.reset();
                query.setDomain("ADMISSIONS", null, null, null);
                query.setField("ADMISSIONS_strAdmissionID", null);
                query.setWhere(null, 0, "ADMISSIONS_intAdmissionkey", "=", Integer.toString(intMaxKey), 0, DALQuery.WHERE_HAS_VALUE);
                ResultSet rs = query.executeSelect();
                
                if (rs.next())
                {    
                    if (rs.getString("ADMISSIONS_strAdmissionID") != null)
                    {    
                        admissionNumber = Integer.parseInt(rs.getString("ADMISSIONS_strAdmissionID"));
                        admissionNumber++;
                    }
                }
        ...
        return (Integer.toString(admissionNumber));
    ...
    public void doAdmissionSmartformsEdit(String patientID, String admissionID, ChannelRuntimeData rd) throws java.sql.SQLException
    {
        ...
        String strNewCollectionGroup = rd.getParameter("ADMISSIONS_strCollectGrp");
        String strNewEpisdoeNum = rd.getParameter("ADMISSIONS_strEpisodeNo");
        int intNumberDistinctGroups = -1;

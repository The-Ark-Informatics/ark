...
/*test*/
import neuragenix.bio.biospecimen.BioAnalysisReport.IDocPreProcessor;
...
public class CBiospecimen implements IChannel {
    ...
    private PickerManager mgrPicker = null;
    ...
    private static boolean AUTO_DOCS = false;
    static
    {
        try {
            AUTO_DOCS = PropertiesManager.getPropertyAsBoolean("neuragenix.bio.report.AutoDocs");
        } catch(Exception e) {
            AUTO_DOCS = false;
            System.out.println("[CBiospecimen] Property neuragenix.bio.report.AutoDocs not present default to false.");
        }
    }
    
    IDocPreProcessor dpp = null;
    IPerson ip = null;
    ...
    public void renderXML(ContentHandler out) throws PortalException {
        ...
        if(this.AUTO_DOCS) {
            upfTmp.setTargetNodeId(SessionManager.getChannelID(strSessionUniqueID, "CTemplateUpdate"));
            xslt.setStylesheetParameter("templateUpdateChannelURL", upfTmp.getUPFile());
            xslt.setStylesheetParameter( "templateUpdateChannelTabOrder", SessionManager.getTabOrder( rp.getAuthToken(), "CTemplateUpdate") );
        }
    ...
    public void setRuntimeData(ChannelRuntimeData rd) throws PortalException {
        runtimeData = rd;
        String viewNotes = "no";
        if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_VIEW_NOTES, rp.getAuthToken())) {
            viewNotes = "yes";
        }
        runtimeData.setParameter("viewnotes", viewNotes);
        ...
        //System.out.println("Biospecimen Channel : Current Module is - " + strModule);
        ...
        if (strModule == null) {
            //System.out.println("Module is null -- Default handler");
            ...
            // Display the previously viewed biospecimen if available
            if (bcBiospecimen.getLastViewedBiospecimenID() != -1) {
                runtimeData.setParameter("BIOSPECIMEN_intBiospecimenID", bcBiospecimen.getLastViewedBiospecimenID() + "");
                strModule = "CORE";
                runtimeData.setParameter("action", "view_biospecimen");
            } else {
        ...
        if (strModule != null) {
            if (strModule.equalsIgnoreCase("NEW_SEARCH")) 
            {
                if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_SEARCH, rp.getAuthToken())) 
                {
                    rp.addXML("<biospecimen>");
            ...
            } else if(strModule.equalsIgnoreCase("AutoDocs")) {
                  /*
            Testing AutoDocs
                   */
                ChannelRuntimeData rntimeData = null;
                if(dpp == null || ((String)rd.getParameter("action")).equalsIgnoreCase("prepare")) {
                    dpp = neuragenix.bio.biospecimen.BioAnalysisReport.ReportFactory.getDocPreProcessorInstance();
                    dpp.setNGXProperties(this.rp);
                    ip.setAttribute("rntimedatastore",new ChannelRuntimeData());
                }
                if(ip.getAttribute("rntimedatastore") != null) {
                    rntimeData = (ChannelRuntimeData)ip.getAttribute("rntimedatastore");
                } else {
                    rntimeData = new ChannelRuntimeData();
                }
                ChannelRuntimeData rdata = dpp.processRuntimeData(rd);
                for(java.util.Enumeration erd=rdata.getParameterNames();erd.hasMoreElements();) {
                    String strkey = (String)erd.nextElement();
                    Object o = rdata.getParameter(strkey);
                    if(o != null) {
                        String value = (String)o;
                        rntimeData.setParameter(strkey,value);
                    } else
                        System.out.println(strkey + ".value=" + o);
                }
                ip.setAttribute("rntimedatastore",rntimeData);
      /*
            Testing AutoDocs
       */
                ...
                if (strAction == null) {
                    ...
                    if (intBiospecimenKey != -1) {
                        ...
                        //LogService.instance().log(LogService.INFO, "<viewnotes>"+runtimeData.getParameter("viewnotes")+"</viewnotes>");
                        ...
                        if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_VIEW, rp.getAuthToken())) {
                            ...
                            if(this.AUTO_DOCS) {
                                rp.addXML("<AutoDocs>Generate Report</AutoDocs>");
                            }
                            rp.addXML("<fromPatientAnalysis>" + runtimeData.getParameter("fromPatientAnalysis")
                            + "</fromPatientAnalysis>");
                    ...
                    //System.out.println("[CBiospecimen] Refresh_view_biospecimen");
                    ...
                    try {
                        ...
                        if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_ADD, rp.getAuthToken())) {
                            ...
                            try {
                                ...
                                if (blLockSet == true) {
                                    strResult = bcBiospecimen.saveBiospecimen(runtimeData);
                    ...
                    if (intBiospecimenKey != -1 && intStudyKey !=-1 ) {
                        ...
                        if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_CLONE, rp.getAuthToken())) {
                            strResult = bcBiospecimen.cloneBiospecimen(intStudyKey, intBiospecimenKey);
                        } else {
                            strResult = ERROR_MESSAGE_PERMISSION_DENIED;
                        }
                        ...
                        //System.out.println("Can not clone because Study Key or Biospecimen Key is invalid");
            ...
            //dealing with notes
                ...
                if (strAction != null) {
                    if (strAction.equals("open_node")) {
                    ...
                    } else if (strAction.equals("close_node")) {
                        int intExpandID = -1;
                        if (htCurrentSearchCriteria != null) // not that it should be
                        {
                            String strCurrentSearchPage = runtimeData.getParameter("PAGING_currentPage");
                        ...
                        htCurrentSearchCriteria.put("BIOSPECIMEN_intStudyKey", runtimeData.getParameter("BIOSPECIMEN_intStudyID"));
                ...
                if (checkPermission(BatchQuantityManager.PERMISSION_BATCH_QUANTITY, rp.getAuthToken())) {
                    if (mgrBatchQuantity == null) {
                        mgrBatchQuantity = new BatchQuantityManager(rp.getAuthToken(), staticData);
                    }
                    rp = mgrBatchQuantity.processRuntimeData(runtimeData);
                } else {
                    rp.setErrorMessage(ERROR_MESSAGE_PERMISSION_DENIED);
                }
...
//            rp.setErrorMessage(ERROR_MESSAGE_PERMISSION_DENIED);
//         }
            } else if (strModule.equalsIgnoreCase("PICKER")) {
                if (mgrPicker == null) {
                    mgrPicker = new PickerManager(rp);
                }
                
                rp = mgrPicker.processRuntimeData(runtimeData);
            }
    ...
    public void setStaticData(ChannelStaticData sd) throws PortalException {
        ...
        if(this.AUTO_DOCS) {
            try {
                strSessionUniqueID = rp.getAuthToken().getSessionUniqueID();
                SessionManager.addSession(strSessionUniqueID);
                SessionManager.addChannelID(strSessionUniqueID, "CTemplateUpdate",
                        (String) globalIDContext.lookup("CTemplateUpdate"));
            } catch (NotContextException nce) {
                LogService.log(LogService.ERROR, "Could not find channel ID for fname=CTemplateUpdate");
            } catch (NamingException e) {
                LogService.log(LogService.ERROR, e);
            }
        }

...
<xsl:include href="./biospecimen_menu.xsl"/>
    ...
    function submittoBatchResultsGeneration ( aURL, value1, value2 ){
        var next = getNextField(); 

        if ((aURL == '') || (value1 == '') || (value2 == ''))
        {
            document.batchSFGeneration.<xsl:value-of select="$currentField" />.value = ' ';
            document.batchSFGeneration.change_next.value = '';
            document.batchSFGeneration.change_next.name = next;
            document.batchSFGeneration.submit();
        }
        else
        {
            document.batchSFGeneration.action.value += aURL;
            document.batchSFGeneration.<xsl:value-of select="$currentField" />.value = value1;
            document.batchSFGeneration.change_next.value = value2;
            document.batchSFGeneration.change_next.name = next;
            document.batchSFGeneration.submit();
        }
	...
	<xsl:if test="(string-length( $fromBatchAnalysis ) = 0)">
                     ...
                     <xsl:when test="$proteinMode = 'SEARCH_PROTEIN'">
                            <!-- <xsl:if test="count(/body/biospecimen/patient_details) &gt; 0"> -->
                                ...
                                <xsl:when test="(string-length( $fromBatchAnalysis ) > 0)">                        
                                    ...
                                    <a href="javascript:submittoBatchResultsGeneration('&amp;formLink=true&amp;PROTEIN_intProteinKey={$PROTEIN_intProteinKey}&amp;{$currentField}={$PROTEIN_strName}&amp;{$nextField}={$PROTEIN_strGene}','{$PROTEIN_strName}', '{$PROTEIN_strGene}')"> 
                                            <xsl:value-of select="PROTEIN_strName"/>
                                    </a>
                                </xsl:when>
                                <xsl:otherwise>
                    ...
                    <td width="33%" class="uportal-label">
                        <xsl:if test="string-length( $lastAction ) = 0">
                            <xsl:if test="count(/body/biospecimen/patient_details) &gt; 0">
                                ...
                                <xsl:variable name="PATIENT_intInternalPatientID"><xsl:value-of select="/body/biospecimen/patient_details/PATIENT_intInternalPatientID"/></xsl:variable>
                                <!--a href="javascript:submitBiospecimen('&amp;PATIENT_intInternalPatientID={$PATIENT_intInternalPatientID}&amp;PROTEIN_intProteinKey={$PROTEIN_intProteinKey}&amp;PROTEIN_strName={$PROTEIN_strName}&amp;PROTEIN_strGene={$PROTEIN_strGene}&amp;PROTEIN_strLocalisation={$PROTEIN_strLocalisation}&amp;currentField={$currentField}&amp;nextField={$nextField}&amp;module=Picker')">
                            ...
                            <xsl:if test="count(/body/biospecimen/patient_details) = 0">
                                ...
                                <!--a href="javascript:submitBiospecimen('&amp;current=search_protein&amp;PROTEIN_intProteinKey={$PROTEIN_intProteinKey}&amp;PROTEIN_strName={$PROTEIN_strName}&amp;PROTEIN_strGene={$PROTEIN_strGene}&amp;PROTEIN_strLocalisation={$PROTEIN_strLocalisation}&amp;currentField={$currentField}&amp;nextField={$nextField}&amp;module=Picker')">
                                     <xsl:value-of select="PROTEIN_strLocalisation"/>
                                </a-->
                                <a>
                                <xsl:value-of select="PROTEIN_strLocalisation"/>
        ...
        <form name="reOrder" action="{$baseActionURL}?current=search_protein&amp;module=Picker" method="post">
           <xsl:for-each select="parameter">  
              <xsl:variable name="name"><xsl:value-of select="name"/></xsl:variable>
              <xsl:variable name="value"><xsl:value-of select="value"/></xsl:variable>
              <input type="hidden" name="{$name}" value="{$value}"/>

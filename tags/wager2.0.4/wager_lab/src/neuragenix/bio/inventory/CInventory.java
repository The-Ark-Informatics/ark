...
public class CInventory implements IChannel,IMimeResponse
{
    ...
    private final String ADD_TRANSFER = "add_transfer";
    ...
    private final String BATCH_UNALLOCATE ="batch_unallocate";
    ...
    private int intSortType = 1; // 1 alphabetically; 2 availbility ; 3 date created						
    ...
    public void setRuntimeData(ChannelRuntimeData rd)
    {
	   ...
	   if(this.invmgr == null)
		...
		System.err.println("strCurrent = " + strCurrent); 
		    ...
		    if (runtimeData.getParameter("target").equals("SITE")){
		   	strCurrentFieldString = "SITE_strSiteID";
                ...
                else 
                {
                    ...
                    else if (strCurrent.equals(ADD_TRANSFER)) 
                    {
                    	int intTrayID = -1;
                    	if (runtimeData.getParameter("TRANSFER_intTrayID") != null)
            				intTrayID =  Integer.parseInt(runtimeData.getParameter("TRANSFER_intTrayID"));
                    	int intSiteKey = InventoryUtilities.getSiteKeyforTray(intTrayID); 
                    	if (!authToken.getSiteList().contains(new Integer(intSiteKey))) {
                    		strXML  = buildSecurityErrorXMLFile("transfer this tray");
                    		strStylesheet = SECURITY;
                    		return;
                    	}
                        ...
                        // display add tray form
                            ...
                            displayAddTransferForm(intTrayID,intSiteKey);
			...
			System.err.println("Called Batch Allocate with intCellID = "+ intCellID + ", " + strBiospecimenID);
		    ...
		    else if (strCurrent.equals(BATCH_UNALLOCATE)) {
		    	int intCellID = -1;
		    	int intTrayID = -1;
		    	if (runtimeData.getParameter("TRAY_intTrayID") != null)
		    		intTrayID = Integer.parseInt(runtimeData.getParameter("TRAY_intTrayID"));
		    	if (runtimeData.getParameter("CELL_intCellID") != null)
					intCellID =  Integer.parseInt(runtimeData.getParameter("CELL_intCellID"));
		    	
		    	String unallocatelist = runtimeData.getParameter("unallocatelist");
		    	System.err.println("Got list: " + unallocatelist);
		    	if (unallocatelist != null) {
		    		DALQuery query = new DALQuery();
	                query.setDomain("BIOSPECIMEN", null, null, null);
	                query.setDomain("CELL","BIOSPECIMEN_intBiospecimenID","CELL_intBiospecimenID","INNER JOIN");
	                query.setFields(DatabaseSchema.getFormFields("cbiospecimen_view_inventory_cell"), null);
	                query.setWhere(null, 0, "CELL_intCellID", "IN", "("+unallocatelist+")", 0, DALQuery.WHERE_HAS_VALUE);
	                ...
	                while (rs.next()) {
	                	int biospecID = rs.getInt("CELL_intBiospecID");
	                	InventoryUtilities.unallocateBiospecimenFromTray(biospecID,authToken);	
	                }
	                rs.close();
		    	
		    	}
		    	displayUnallocateForm(authToken,intTrayID);
    ...
    private String buildXMLInventoryAdmin()
    {
        ...
        try 
        {
        	...
        	if (authToken.getSiteList().size() != 0) {
        		...
        		if (authToken.hasActivity("inventory_addTank") && authToken.getSiteList().size() > 0)
        		{
        			theXML += "<intInventoryAddTank>1</intInventoryAddTank>";
        			adminRequired = true;
        		}

        		if (authToken.hasActivity("inventory_addBox") && authToken.getSiteList().size() > 0)
        		{
        			theXML += "<intInventoryAddBox>1</intInventoryAddBox>";
        			adminRequired = true;
        		}

        		if (authToken.hasActivity("inventory_addTray") && authToken.getSiteList().size() > 0)
        		{
	...
	//rennypv            lockRequest = new LockRequest(authToken);
	            ...
	            String studyInStr = StudyUtilities.getStudyIDSQLString(authToken.getGroupsForActivity("inventory_viewforeign"));
	            String siteInStr = StudyUtilities.getStudyIDSQLString(authToken.getSiteList());
	            query.setDomain("STUDYINV","SITE_intSiteID","STUDYINV_intSiteID","LEFT JOIN");
	            Vector vtFormFields = DatabaseSchema.getFormFields("cinventory_list_inventory_site");
	            query.setWhere(null, 0, "STUDYINV_intStudyID", "IN", studyInStr, 0, DALQuery.WHERE_HAS_VALUE);
	            query.setWhere("OR",0, "STUDYINV_intDestID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	            query.setWhere("OR",0, "SITE_intSiteID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	...
	/*                if (strCurrentNode.equals("SITE_" + intSiteID))
	            ...
	            query.setDomain("STUDYINV","TANK_intTankID","STUDYINV_intTankID","LEFT JOIN");
	            vtFormFields = DatabaseSchema.getFormFields("cinventory_view_inventory_tank");
	            query.setFields(vtFormFields, null);
	            query.setWhere(null, 0, "STUDYINV_intStudyID", "IN", studyInStr, 0, DALQuery.WHERE_HAS_VALUE);
	            query.setWhere("OR",0, "STUDYINV_intDestID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	            query.setWhere("OR",0, "TANK_intSiteID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	            query.setDistinctQuery();
	...
	/*                            if (hashExpanded.containsKey("SITE_" + hashNewNode.get("TANK_intSiteID").toString()))
	            ...
	            query.setDomain("STUDYINV","BOX_intBoxID","STUDYINV_intBoxID","LEFT JOIN");
	            query.setDomain("TANK","TANK_intTankID","BOX_intTankID","INNER JOIN");
	            query.setDomain("SITE","SITE_intSiteID","TANK_intSiteID","INNER JOIN");
	            query.setWhere(null, 0, "STUDYINV_intStudyID", "IN", studyInStr, 0, DALQuery.WHERE_HAS_VALUE);
	            query.setWhere("OR",0, "SITE_intSiteID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	            query.setWhere("OR",0, "STUDYINV_intDestID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	            query.setDistinctQuery();
	...
	/*                            if (hashExpanded.containsKey("TANK_" + hashNewNode.get("BOX_intTankID").toString()))
	            ...
	            query.setDomain("STUDYINV","TRAY_intTrayID","STUDYINV_intTrayID","LEFT JOIN");
	            query.setDomain("BOX","BOX_intBoxID","TRAY_intBoxID","INNER JOIN");
	            query.setDomain("TANK","TANK_intTankID","BOX_intTankID","INNER JOIN");
	            query.setDomain("SITE","SITE_intSiteID","TANK_intSiteID","INNER JOIN");
	            query.setWhere(null, 0, "STUDYINV_intStudyID", "IN", studyInStr, 0, DALQuery.WHERE_HAS_VALUE);
	            query.setWhere("OR",0, "STUDYINV_intDestID", "IN", siteInStr,0,DALQuery.WHERE_HAS_VALUE );
	...
	//add sort type
        switch (node.getLevel())
        {
            ...
            case 1:
                ...
                Integer IntSiteID = new Integer(""+ hashTemp.get("SITE_intSiteID"));
                if (authToken.getSiteList().contains(IntSiteID)) {
                	strResult += "<siteAccess>1</siteAccess>";
                }
                else
                	strResult += "<siteAccess>0</siteAccess>";
            ...
            case 4:
                ...
                int transferID = -1;
                if (hashTemp.get("TRAY_intTransferID") != null) {
                	 transferID = new Integer(hashTemp.get("TRAY_intTransferID").toString()).intValue();
               ...
               if (transferID != -1) {
            	   try {
            	   DALQuery query = new DALQuery();
            	   query.setDomain("TRANSFER",null,null,null);
            	   query.setDomain("SITE","SITE_intSiteID","TRANSFER_intSiteID","INNER JOIN");
            	   query.setWhere(null, 0, "TRANSFER_intPendID", "=", ""+transferID, 0, DALQuery.WHERE_HAS_VALUE);
            	   query.setWhere("AND", 0, "TRANSFER_intDeleted", "=", "0", 0, DALQuery.WHERE_HAS_VALUE);
            	   Vector vtAddTransferFields = DatabaseSchema.getFormFields("cinventory_add_transfer");
            	   vtAddTransferFields.add("SITE_strSiteName");
            	   query.setFields(vtAddTransferFields, null);
            	   ResultSet rsResultSet = query.executeSelect();
            	   strResult += QueryChannel.buildSearchXMLFile("transfer",  rsResultSet, vtAddTransferFields); 
    ...
    private DefaultMutableTreeNode filterInventoryTree(DefaultMutableTreeNode root, AuthToken authToken) {
    	try {
    	Vector groups = authToken.getGroupsForActivity("INVENTORY_VIEW");
    	String inString="";
    	if (groups.size() == 0)
    		inString = "(null)";
    	else { 
	...
	int intSiteID = Integer.parseInt(runtimeData.getParameter("SITE_intSiteID"));
        ...
        try
        {
            if((strObjExistsError = this.invmgr.objectExists("SITE",runtimeData.getParameter("SITE_intSiteID"))).length() == 0)
            {
                ...
                if (this.authToken.hasSite(intSiteID)) {
...
// LogService.instance().log("strXML = " + strXML);
        ...
        try
        {
            ...
            if (strCheckRequiredFields == null && strCheckDuplicates.length() == 0)
            {
                if (lockRequest.isValid() && lockRequest.lockWrites())
                {
                    ...
                    if (this.authToken.hasSite(intSiteID)) {
                    	strXML += "<hasEditRights>1</hasEditRights>";
                    }
                }
                else
                    strXML = QueryChannel.buildFormLabelXMLFile(vtUpdateSiteFormFields) +
    ...
    private void displayAddTankForm()
    {
        ...
        try
        {
            ...
            String strSiteSQL = StudyUtilities.getStudyIDSQLString(authToken.getSiteList());
            query.setWhere(null,0,"SITE_intSiteID", "IN",strSiteSQL,0,DALQuery.WHERE_HAS_VALUE);
	...
	int intTankID = Integer.parseInt(runtimeData.getParameter("TANK_intTankID"));
        ...
        try
        {
            if((strObjExistsError = this.invmgr.objectExists("TANK",runtimeData.getParameter("TANK_intTankID"))).length() == 0)
            {
            	...
            	int intSiteID = InventoryUtilities.getSiteKeyforTank(intTankID);
                    ...
                    if (strCheckRequiredFields == null && strCheckDuplicates.length() == 0)
                    {
                        ...
                        if (strValidation == null)
                        {
                            ...
                            if (lockRequest.isValid() && lockRequest.lockWrites())
                            {
                                ...
                                rsSiteList.close();
                                int intSiteID = InventoryUtilities.getSiteKeyforTank(intCurrentTankID);
                            	if (authToken.getSiteList().contains(new Integer(intSiteID)))
                            		strXML = strXML + "<hasEditRights>1</hasEditRights>";
                            }
                ...
                if (strCheckRequiredFields == null && strCheckDuplicates.length() == 0)
                {
                    if (lockRequest.isValid() && lockRequest.lockWrites())
                    {
                        ...
                        strXML = QueryChannel.buildSearchXMLFile("search_tank", rsTankList, vtListTankFormFields) +
                                 ...
                                 "<blLockError>false</blLockError>";
                        rsTankList.close();
                        int intSiteID = InventoryUtilities.getSiteKeyforBox(intCurrentBoxID);
                    	if (authToken.getSiteList().contains(new Integer(intSiteID)))
                    		strXML = strXML + "<hasEditRights>1</hasEditRights>";
                    }
                    else
    ...
    private void doAddTransfer() {
    	Vector vtAddTransferFields = DatabaseSchema.getFormFields("cinventory_add_transfer");
    	try
    	{
    		int intTrayID =  new Integer(runtimeData.getParameter("TRANSFER_intTrayID")).intValue();
    		
    		this.setLocking("TRANSFER", runtimeData.getParameter("TRANSFER_intTrayID"));
    		this.setLocking("TRAY", runtimeData.getParameter("TRANSFER_intTrayID"));
    		if(this.lockRequest.isValid() && this.lockRequest.lockWrites())
    		{
    			strStylesheet = VIEW_BOX;
    			runtimeData.setParameter("TRANSFER_strUsername", authToken.getUserIdentifier());

                DALSecurityQuery query = new DALSecurityQuery(INVENTORY_INSERT, authToken);

                // add new box
                query.setDomain("TRANSFER", null, null, null);    
                query.setFields(vtAddTransferFields, runtimeData);
                query.executeInsert();
                int intCurrentTransferID = QueryChannel.getNewestKeyAsInt(query);
                query.reset();
                query.setDomain("TRAY", null, null, null);
                query.setField("TRAY_intTransferID",  intCurrentTransferID + ""); // associate the biospecimen to cell
                ...
                this.lockRequest.unlockWrites();   
                runtimeData.setParameter("TRAY_intTrayID",runtimeData.getParameter("TRANSFER_intTrayID"));
                displayViewTrayForm();
    		}
    		else
    		{
    ...
    private void displayAddTransferForm(int intTrayID, int intSiteKey)
    {
    	strStylesheet = ADD_TRANSFER;
    	Vector vtAddTransferFormFields = DatabaseSchema.getFormFields("cinventory_add_transfer");
    	   Vector vtListSiteFormFields = DatabaseSchema.getFormFields("cinventory_list_inventory_site");
    	   try{
    	   DALSecurityQuery query = new DALSecurityQuery(INVENTORY_VIEW, authToken);
    	query.setDomain("SITE", null, null, null);
        query.setFields(vtListSiteFormFields, null);
        query.setOrderBy("SITE_strSiteName", "ASC");
        query.setWhere(null,0,"SITE_intSiteID", "<>",""+intSiteKey,0,DALQuery.WHERE_HAS_VALUE);
        ResultSet rsSiteList = query.executeSelect();
    	
    	
    	strXML =  QueryChannel.buildSearchXMLFile("search_site", rsSiteList, vtListSiteFormFields) + 
    	QueryChannel.buildFormLabelXMLFile(DatabaseSchema.getFormFields("cinventory_view_titles")) +
        QueryChannel.buildFormLabelXMLFile(vtAddTransferFormFields);
    	strXML += "<intCurrentSite>" + intSiteKey + "</intCurrentSite>";
    	strXML += "<intCurrentTray>" + intTrayID + "</intCurrentTray>";
    ...
    private void doUpdateTray()
    {
        ...
        String strObjExistsError = "";
        boolean clearPendTransfer = false;
                    ...
                    if (strValidation == null)
                    {
                        ...
                        if (intNewNoOfRow > 0 && intNewNoOfRow < 27 && intNewNoOfCol >0 && intNewNoOfCol < 27)
                        {
                            if (lockRequest.isValid() && lockRequest.lockWrites())
                            {
                                ...
                                int intOldSiteID = InventoryUtilities.getSiteKeyforTray(intCurrentTrayID);
                                int intNewSiteID = InventoryUtilities.getSiteKeyforBox(intNewBoxID);
                                
                                if(intOldSiteID != intNewSiteID) {
                                	// Right we are changing sites here - is it because of a transfer?
                                	if (authToken.getSiteList().contains(new Integer(intNewSiteID))) { // If not due to legit site transfer. 

                                		int newSiteTransferKey = -1;
                                		DALQuery txquery = new DALQuery();
                                		txquery.setDomain("TRANSFER",null,null,null);
                                		txquery.setField("TRANSFER_intSiteID", null);
                                		txquery.setWhere(null, 0, "TRANSFER_intTrayID", "=",""+intCurrentTrayID , 0, DALQuery.WHERE_HAS_VALUE);
                                		txquery.setWhere("AND", 0, "TRANSFER_intDeleted", "=","0" , 0, DALQuery.WHERE_HAS_VALUE);
                                		ResultSet txResult = txquery.executeSelect();
                                		if (txResult.next()) {
                                			newSiteTransferKey = txResult.getInt("TRANSFER_intSiteID");
                                		}
                                		txResult.close();
                                		if (!authToken.getSiteList().contains(new Integer(newSiteTransferKey)) || newSiteTransferKey == -1) {
                                			strErrorMessage = "You do not have permission to move this tray.";
                                			displayViewTrayForm();
                                			return;
                                		}
                                		else
                                			clearPendTransfer = true;
                                ...
                                if (clearPendTransfer) {
                                	query.reset();
                                	query.setDomain("TRANSFER", null, null, null);
                                	query.setField("TRANSFER_intDeleted","-1");
                                	 query.setWhere(null, 0, "TRANSFER_intTrayID", "=", runtimeData.getParameter("TRAY_intTrayID"), 0, DALQuery.WHERE_HAS_VALUE);
                                     query.executeUpdate();
                                }
                                // set the lock to READ_LOCK
                                lockRequest.unlockWrites();
                ...
                for (int i=0; i < intNoOfRow + 1; i++)
                {
                    ...
                    for (int j=0; j < intNoOfCol + 1; j++)
                    {
                        ...
                        else
                        {
                            ...
                            int intWithdrawn =rsResultSet.getInt("BIOSPECIMEN_intWithdrawn");
                            ...
                            strXML += "<CELL_intWithdrawn>" + intWithdrawn + "</CELL_intWithdrawn>";
 ...
 private void updateHashExpanded(String key, boolean fromLink)
    ...
    private String buildSecurityErrorXMLFile(String aFunctionalArea)
    {
        String strTempErrorXML  = "<?xml version=\"1.0\" encoding=\"utf-8\"?><securityerror>"
                ...
                + "<errortext>You" 
                + " are not authorised to " + aFunctionalArea + "</errortext>"
                + "<errordata></errordata>"
                + "</securityerror>";
    ...
    private void displayUnallocateForm(AuthToken authToken, int intTrayID) {
    	try
            {
    		strStylesheet = BATCH_UNALLOCATE;
    	int intCurrentTrayID = Integer.parseInt(runtimeData.getParameter("TRAY_intTrayID")); 
    	 /* DALSecurityQuery query = new DALSecurityQuery(INVENTORY_VIEW, authToken);
                query.setDomain("CELL", null, null, null);
    	    query.setDomain("TRAY", "CELL_intTrayID", "TRAY_intTrayID", "INNER JOIN");
...
//    		strXML += "<CELL_strRow>"+strRowNo+"</CELL_strRow>";
//    		strXML += "<CELL_strCol>"+strColNo+"</CELL_strCol>";
    		System.err.println("Calling with: " +intCurrentTrayID);
    	}
    	catch (Exception e)
            {
                LogService.instance().log(LogService.ERROR, "Unknown error in Inventory Channel - " + e.toString(), e);

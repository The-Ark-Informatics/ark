...
public class IntelligentSearch 
{
    ...
    private String strDelimiter = "tab";
    ...
    private String buildStudyListXMLFile(){
        ...
        for( int i=0; i < vtStudyList.size(); i++ ){
            ...
            strResult+= "<study>"+
                        "<strDomainName>"+ Utilities.cleanForXSL(studyName) +"</strDomainName>" +
                        "</study>";
        }
    ...
    private boolean isSelectedStudy( String strStudyName ){
        ...
        for( int i=0; i < vtSelectedStudys.size(); i++ )
            if( strStudyName.equals(vtSelectedStudys.get(i).toString()) )
                return true;
                ...
                // check if it is a smartform
                 ...
                 if( !vtSelectedDomains.contains("PATIENT") && strSelectedDomain.equals("BIOSPECIMEN")){
                     ...
                     for(int i =0; i < vtSelectedStudys.size(); i++){
                        ...
                        hashDomainStatus.remove( vtSelectedStudys.get(i).toString() );
                        hashCriteriaDomains.remove(vtSelectedStudys.get(i).toString() );
                        vtCriteriasDomains.remove( vtSelectedStudys.get(i).toString() );
                     }
   ...
   try{
    ...
    //       if(  vtSelectedDomains.isEmpty() == false && vtSelectedStudys.isEmpty() == false ){
      ...
      //     }
           ...
           //Set the study restrictions first.
           
           //*******Chris Williams:
           // Added restrictions here to studies - pull the list from authToken.
           vtSelectedStudys = authToken.getStudyList();
           int intSelectedStudysSize = vtSelectedStudys.size();
            // if study is selected, add it to the where condition
           for( int j=0; j < vtSelectedStudys.size(); j++){
                if(j == 0){
                	if (vtSelectedStudys.size() == 1)
                		query.setWhere( null, 0, "STUDY_intStudyID", "=", vtSelectedStudys.get(j).toString(), 0, DALQuery.WHERE_HAS_VALUE );
                	else
                		query.setWhere( null, 1, "STUDY_intStudyID", "=", vtSelectedStudys.get(j).toString(), 0, DALQuery.WHERE_HAS_VALUE );
                }else{
                   
                    if(j == vtSelectedStudys.size() -1) {
                    	query.setWhere( "OR", 0, "STUDY_intStudyID", "=", vtSelectedStudys.get(j).toString(), 1, DALQuery.WHERE_HAS_VALUE );
                    }
                    else 
                    {
                        query.setWhere( "OR", 0, "STUDY_intStudyID", "=", vtSelectedStudys.get(j).toString(), 0, DALQuery.WHERE_HAS_VALUE );
                    }    
                }
           }
           ...
           for(int i=0; i < vtJoin.size(); i++){ 
                ...
                if( vtOp.get(i).toString().equals("Is empty") || vtOp.get(i).toString().equals("Is not empty")){
                    ...
                    if( vtJoin.get(i).toString().equals("EXCLUDING")){
                        ...
                        if(i == 0) {
                        	if (intSelectedStudysSize != 0){
                        		 query.setWhere( "AND", intOpenBracket, strIntlName, strOp, strSearchVal, intCloseBracket, DALQuery.WHERE_HAS_NULL_VALUE );
                        }else
                    ...
                    if(i == 0) {
                    	if (intSelectedStudysSize != 0){
                    		  query.setWhere( "AND", intOpenBracket, strIntlName, strOp, vtSearchVal.get(i).toString(), intCloseBracket, DALQuery.WHERE_HAS_VALUE );
                    }else
   ...
   protected String dbOperator(String op){
            ...
            // if criteria is not updated and save is clicked
                ...
                for(int k = 0; k < vtSelectedStudys.size(); k++){                    
                    ...
                    if(strSeltdStudyList.equals("")){
                        strSeltdStudyList =  vtSelectedStudys.get(k).toString() + ",";
        ...
        try{
            ...
            if(rs.next()){
                ...
                for ( int n =0; n < vtSelectedStudys.size(); n++ ){                                 
                    hashDomainStatus.put( vtSelectedStudys.get(n).toString(), strDomainVisibility);                            
                }  
    ...
    private void createExportFileForSmartforms()
    {
        ...
        {      
            ...
            strFileName = authToken.getSessionUniqueID() + "exportfile.psv";
    ...
    private void createExportFile()
    {
        try
        {
            ...
            strFileName = authToken.getSessionUniqueID() + "ABN_QueryResult_1.csv";
            ExportFile = new BufferedWriter(new FileWriter(new File(EXPORT_DIRECTORY + "/" + strFileName)));
            String column = "";  
            ...
            {          
                if( !hashNonDisplayFields.contains(vtSearchFormFields.get(j).toString()) ){
                    ...
                    String data = Utilities.cleanForExcelFile(field.getLabelInForm());                    
                    column += data + strDelimiter;
                }
            }
            ExportFile.write(column, 0, column.length() - 1);
    ...
    private void createFile(ResultSet rs){
        ...
        try{
        String filename = authToken.getSessionUniqueID() +"Anita.psv";
    ...
    private void LoadStudysSmartforms(){        
        try{       
            ...
            for(int i=0; i < vtSelectedStudys.size(); i++){
                ...
                studyName = vtSelectedStudys.get(i).toString();
                DALQuery query = new DALQuery();
                query.setDomain("STUDY", null, null, null);
...
//Seenadist               query1.setDistinctField( strDistinct );     
       ...
       System.err.println(query1.convertSelectQueryToString());
        }catch(Exception e){  
            LogService.instance().log(LogService.ERROR, "Unknown error in ISearch Channel - " + e.toString(), e);           
        } 

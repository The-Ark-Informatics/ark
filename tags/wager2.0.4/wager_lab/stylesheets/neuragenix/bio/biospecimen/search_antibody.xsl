			...
			Select Antibody
                        ...
                        <td class="uportal-label" align="right">
                            <xsl:if test="count(/body/biospecimen/patient_details) &gt; 0">
                                ...
                                <input type="button" value="&lt; Back" class="uportal-button" onclick="javascript:submitBiospecimen('&amp;PATIENT_intInternalPatientID={$PATIENT_intInternalPatientID}&amp;currentField={$currentField}&amp;nextField={$nextField}')"/>
                            </xsl:if>
                            <xsl:if test="count(/body/biospecimen/patient_details) = 0">
                                <!--input type="submit" value="View study results" tabindex="42" class="uportal-button" /-->
                                <input type="button" value="&lt; Back" class="uportal-button" onclick="javascript:submitBiospecimen('&amp;currentField={$currentField}&amp;nextField={$nextField}')"/>
                            </xsl:if>  
                        </td>
		</tr>
                <tr>

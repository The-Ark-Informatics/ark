...
<!--
    Document    : ecgcodes.xsl
    Created on  : September 20, 2004, 3:12 PM
    Author      : Daniel Muyrley
    Email       : dmurley@neuragenix.com
    Description : Displays a list of ecg codes based on what has been loaded into
...
<xsl:template match='ecgcodes_picker'>
...
dTree.prototype.node = function(node, nodeId) {
	...
	var str = '&lt;div class="dTreeNode"&gt;';
        ...
        // if( a != b ){
            ...
            str += '&lt;input id="ECGCODES" type="checkbox" name="ecgcodes_' + node.id + '" value="true" ';
            if( node.selected )
                str +='checked="true"';
...
dTree.prototype.select = function(id, status) {
        ...
        if( status ){
            ...
            if( (this.aNodes[n].pid != this.root.id) ){
                ...
                eval( 'document.ecgcodesForm.ecgcodes_' + this.aNodes[n].id + '.checked = true');
...
function submitSelected(){
    document.ecgcodesForm.action = document.ecgcodesForm.action + '&amp;ecgcodesSelected=true';
    document.ecgcodesForm.submit();
...
function submitSelected( currentField ){
    ...
    var ecgCodesCount = parseInt( document.getElementById( 'ecgCodesCount' ).getAttribute('value') ) ;
    
    var strValues = '';
    for( var i = 1; i &lt;= ecgCodesCount; i++ ){

        var checkObj = eval( 'document.ecgcodesForm.ecgcodes_' + i );
        
        
        if( checkObj ){

            if( checkObj.checked ){
                strValues += document.getElementById( 'ecgcodes_' + i ).getAttribute( 'nameValue' ) + "\n" ;
            }else{
    ...
    var currentObj = eval( "document.ecgcodesForm." + currentField );
        ...
        <table width="100%">
			...
			Choose ECG Code<br/><hr/>
                ...
                <tr>
                    ...
                    <xsl:variable name="ecgCodesCount"><xsl:value-of select="ecgCodesCount"/></xsl:variable>
                    
                    <span id="ecgCodesCount" value="{$ecgCodesCount}"/>
                    <xsl:for-each select="ecgcodes">
                        
                       
                    
                        <xsl:variable name="ECGCODES_intECGCodeKey"><xsl:value-of select="ECGCODES_intECGCodeKey"/></xsl:variable>
                        <xsl:variable name="ECGCODES_strECGCodeName"><xsl:value-of select="ECGCODES_strECGCodeName"/></xsl:variable>
                        <xsl:variable name="ECGCODES_strECGCode"><xsl:value-of select="ECGCODES_strECGCode"/></xsl:variable>
                        <xsl:variable name="ECGCODES_intParentKey"><xsl:value-of select="ECGCODES_intParentKey"/></xsl:variable>
                        
                    
                        <span id="ecgcodes_{$ECGCODES_intECGCodeKey}" keyValue="{$ECGCODES_intECGCodeKey}"
                            nameValue="{$ECGCODES_strECGCode}: {$ECGCODES_strECGCodeName}" codeValue="{$ECGCODES_strECGCode}" 
                            parentValue="{$ECGCODES_intParentKey}">
                    ...
                    <form name="ecgcodesForm" action="{$baseActionURL}?uP_root=root&amp;" method="post">
                    ...
                    -->
                        ...
                        for( var i = 1; i &lt;= ecgCodesCount; i++ ){
                            ...
                            if( document.getElementById( 'ecgcodes_' + i ) ){

                                d.add(document.getElementById( 'ecgcodes_' + i ).getAttribute( 'keyValue') ,
                                document.getElementById( 'ecgcodes_' + i ).getAttribute( 'parentValue'),
                                document.getElementById( 'ecgcodes_' + i ).getAttribute( 'nameValue'),
                                document.getElementById( 'ecgcodes_' + i ).getAttribute( 'codeValue'),
                                document.getElementById( 'ecgcodes_' + i ).getAttribute( 'selectedValue'),'');
            ...
            <tr>
                ...
                <td align="right">
                   <form action="{$baseActionURL}?uP_root=root&amp;" method="POST">
                        ...
                        <input type="button"  name="ecgcodesSelected" value="Submit" onclick="javascript:submitSelected('{$currentField}')" class="uportal-button" />
                    </form>
                </td>
            </tr>
        </table>

...
<xsl:template match="isearch">
    ...
    <a name="frmbuild_isearch_anchor"/>
        ...
        <table width="100%">
            <tr>
                <td class="uportal-channel-subtitle">
                    Basic Search - Simple Mode<br/><hr/>                   
                </td> 
            </tr> 
        </table>
        ...
        <!-- Only display this area if there are Search or smartform criteria to begin with -->
        <xsl:if test="(string-length(searchCriteria) > '0' or string-length(smartformCriteria) > '0')">
        ...
        <xsl:when test="(string-length($visibility1) > '0' and ($visibility1= 'expanded')) or ($edit = 'true')">   
            ...
            <table width="100%">
                 ...
                 <xsl:if test="$target = 'editCriteria' or $number != 'null'">                        
                     <tr class="uportal-label">

                        <td width="10%">Select the field</td>
                        <td width="10%">Specify the type of matching</td>
                        <td width="10%">Specify the search value</td>
                        <td width="70%"></td>                        
                    </tr>
                    ...
                    <input type="hidden" name="openBracket" value="{$openBracketSel}"/>
                    <input type="hidden" name="join" value="{$joinSel}"/>
                    <input type="hidden" name="closeBracket" value="{$closeBracketSel}"/>
                    <input type="hidden" name="participantDomain" value="{$participantDomainSel}"/>
                           ...
                           <td width="10%" class="uportal-input-text">
                            ...
                            <td width="5%" class="uportal-input-text">
                                <xsl:if test="function-available('url:encode')">
                                    <a href="#" onclick="javascript:mySubmit('&amp;target=editCriteria&amp;fieldSel={$fieldSel}&amp;SelectedDomainSel={$domainVal}&amp;selFieldType={$selFieldType}&amp;opSel={$opSel}&amp;searchValue={url:encode($searchValue)}&amp;joinSel={$joinSel}&amp;number={$criteriaNo}&amp;labelInColumn={$labelInColumn}&amp;openBracketSel={$openBracketSel}&amp;closeBracketSel={$closeBracketSel}&amp;participantDomainSel={$participantDomainSel}')">Edit</a>
                                </xsl:if>  
                            </td>                      
                            <td width="5%" class="uportal-input-text">
...
-->                        
        ...
        <!-- Only display this area if there are Missing records criteria to begin with -->
        <xsl:if test="(string-length(searchMissingCriterias) > '0')">
        ...
        <table width="100%">
            <xsl:if test="$targetMissing = 'editCriteria'">
                 <tr>
                    <td width="100%" class="uportal-label">
                        Search for &#160;&#160;
                   </td>
...
-->                          
        ...
        <!-- Only display this area if there are Missing records criteria to begin with -->
        <xsl:if test="(string-length(searchMissingSFCriterias) > '0')">
        ...
        <table width="100%">
            <xsl:if test="$targetMissingSF = 'editCriteria'">
                 <tr>
                   <td width="100%" class="uportal-label">

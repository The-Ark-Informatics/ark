...
public class BiospecimenCore {
    ...
    public String saveBiospecimen(ChannelRuntimeData runtimeData ) {
        ...
        try {
            ...
            if (runtimeData.getParameter("BIOSPECIMEN_intParentID")!= null && runtimeData.getParameter("BIOSPECIMEN_intParentID").toString().length()>0) {
                strInternalParentKey = runtimeData.getParameter("BIOSPECIMEN_intParentID");
		...
		int intParentBioSpecID = 0;	
                ...
                strBiospecimenID =  runtimeData.getParameter("BIOSPECIMEN_strBiospecimenID");
                strInternalBiospecimenID = runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID");
               ...
               if (runtimeData.getParameter("BIOSPECIMEN_intParentID")!= null && runtimeData.getParameter("BIOSPECIMEN_intParentID").toString().length()>0) {
		intParentBioSpecID = Integer.parseInt(strInternalParentKey);
		DALSecurityQuery query =  new DALSecurityQuery("biospecimen_add", authToken);
		strNewBiospecimenID = BiospecimenUtilities.getNewSubBiospecimenStringID(query, BiospecimenUtilities.getUserBiospecimenID(intParentBioSpecID),0);
 } else {
		...
		doSaveQuantity(strNewBiospecimenID, runtimeData);
                        }
                        else {
                ...
                // i believe that study will be discontinured so the newly added code below will be commented. code copy from the same class in bluefire/chw-cardiac client
                /*
                 int intStudyKey = -1;
                try {
                ...
                //use the 
                if (SmartformManager.checkForBiospecimenParticipant(this.intLastBiospecimenAddedID, intStudyKey) == false) {
                    System.out.println(intLastBiospecimenAddedID + " - " + intStudyKey);
			    ...
			    java.sql.Time tmObject = null;
                            ...
                            if (field.getDataType() == DBMSTypes.DATE_TYPE ){
                                ...
                                if (tmObject != null) {
                                    ...
                                    htBiospecimenDetails.put(strFieldName, dfFormatter.format(tmObject));
                                }
                            }
                            else
    ...
    
    {
        ...
        if (strBiospecimenKeys == null || strBiospecimenKeys.trim().length()==0)
        ...
        try {
            ...
            dbCurrentQuantity = dbCurrentQuantity - numberDeleted;
            dbRemoved = dbRemoved + numberDeleted;
            rsResult.close();
             
                // update bio quantity
            query.reset();
            query.setDomain("BIOSPECIMEN", null, null, null);
            query.setField("BIOSPECIMEN_flNumberCollected", String.valueOf(dbCurrentQuantity));
            query.setField("BIOSPECIMEN_flNumberRemoved", String.valueOf(dbRemoved));
             
            query.setWhere(null, 0, "BIOSPECIMEN_intBiospecimenID", "=", intBiospecimenKey + "", 0, DALQuery.WHERE_HAS_VALUE);
            query.executeUpdate();
             */
            
            dsqDeleteVialCalculation = new DALSecurityQuery("biospecimen_delete", authToken);
            ...
            //dsqDeleteBiospecimen = new DALSecurityQuery("biospecimen_delete", authToken);
            int counter=0;
    ...
    
    {
        ...
        try {
            ...
            //Query transaction
            DALSecurityQuery dsqGetTransaction = new DALSecurityQuery("biospecimen_delete", authToken);
            dsqGetTransaction.setDomain("BIOSPECIMEN_TRANSACTIONS", null, null, null);
            dsqGetTransaction.setField("BIOSPECIMEN_TRANSACTIONS_intBioTransactionID", null);
            dsqGetTransaction.setWhere(null, 0, "BIOSPECIMEN_TRANSACTIONS_intBiospecimenID", "=", intBiospecimenKey + "", 0, DALQuery.WHERE_HAS_VALUE);
            
            //LogService.instance().log(LogService.INFO, dsqGetTransaction.convertSelectQueryToString());
        ...
        try {
            ...
            if (runtimeData != null) {
                ...
                if ( strSampleType != null){
                    ...
                    if (runtimeData != null && runtimeData.get("keepSubType") != null &&
                    runtimeData.getParameter("keepSubType").equalsIgnoreCase("true")) {
                        strCurrentSubType = runtimeData.getParameter("BIOSPECIMEN_strSampleSubType");
                        //build SubType LR
                        if (blUseSubTypeLR) {
                            strXML.append(QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSubTypeLR", "", strCurrentSubType));
                        }
                    }
                    else {
                        String strFirstSubType = QueryChannel.getFirstItemInLOVs("BIOSPECIMEN_strSampleSubType", strSampleType);
                        //build Subtype LR
                        if (strFirstSubType != null) {
                            if (blUseSubTypeLR) {
                                strXML.append(QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSubTypeLR", "", strFirstSubType));
                            }
                        }
                    }
            ...
            strXML.append(QueryChannel.buildFormLabelXMLFile(DatabaseSchema.getFormFields("cbiospecimen_inventory_titles")));
    ...
    */
   public String getBiospecimenWADBXML(int intBiospecimenKey, ChannelRuntimeData runtimeData) {
	StringBuffer strXML = new StringBuffer();

	try {
 Vector vtViewBiospecimen = (Vector)DatabaseSchema.getFormFields("cbiospecimen_wadb_allocations").clone();
	   ...
	   dsqBiospecimenView.setWhere(null, 0, "WADBVIEW_intBiospecimenID", "=", intBiospecimenKey + "", 0, DALQuery.WHERE_HAS_VALUE);
	   System.out.println(dsqBiospecimenView.convertSelectQueryToString());
           ResultSet rs = dsqBiospecimenView.executeSelect();
            ...
            //}
	    //strXML.append(QueryChannel.buildViewXMLFile(vtViewBiospecimen, runtimeData));
	    strXML.append(QueryChannel.buildSearchXMLFile("wadb_view_list", rs, vtViewBiospecimen));
            ...
            if(isRefresh) {
                ...
                if (strSampleType != null && strSampleSubType != null) {
                    ...
                    if (strSampleSubTypeLR != null) {
                        if (blUseSubTypeLR) {
                            strXML.append(QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSubTypeLR", strSampleSubTypeLR, strSampleSubType));
                        }
        ...
        **/
        ...
        if (intExpandID != -1 && blNewSearch == false) {
            ...
            try {
                ...
                dalChildQuery.setOrderBy("BIOSPECIMEN_strBiospecimenID", "ASC", true); 

...
function printBarcode(printerName, barcodeString) {
	...
	if (applet != null) {
		...
		//alert("string =" + barcodeString);
		
		//unescapedString = unescape(barcodeString);
		//alert("unescaped string =" + unescapedString);
		//applet.append(unescapedString);
/* this is what we used to send
string =D14%0AN%0Ab200,15,D,h3,c16,r16,%220%22%0AA100,20,0,2,1,1,N,%220%22%0AA100,40,0,2,1,1,N,%22      %22%0AA115,60,0,2,1,1,N,%22    %22%0AA260,15,0,1,1,2,N,%220%22%0AA260,45,0,1,1,2,N,%22%22%0AP1%0A%0A
unescaped string =D14
N
b200,15,D,h3,c16,r16,"0"
A100,20,0,2,1,1,N,"0"
A100,40,0,2,1,1,N,"      "
A115,60,0,2,1,1,N,"    "
A260,15,0,1,1,2,N,"0"
...
//$XA=startFormat  ^FO=FieldOrigin  ^A=ScalableBitmapFont--but probably is actually the next line...  
//^ADN,n,m command, where n and m are integers; ^ADN,18,10 is the smallest size and ^ADN,180,100 the largest. 
//^BY is barcode Field Default  //applet.append('^B2N,150,Y,N,N');B2N bar code...worked but big...and Chris recommends Data Matric Barcode BX
	
	//$XA=startFormat - note that start format is start job also in other documentation
		
		
		applet.append('^XA\n');
		applet.append('^FO-10,20^BXN,10,100\n');
		applet.append('^FD12345^FS\n');
//		applet.append('^FO110,20^ADN,36,20^FDIRD-123456\n^FS\n');
//		applet.append('^FO110,50^ADN,36,20^FD07/07/1977\n^FS\n');seems to have a 9:5 ratio on font height and width
		applet.append('^FO100,20^ADN,50,27^FDIRD-123456\n^FS\n');
		applet.append('^FO100,70^ADN,50,27^FD07/07/1977\n^FS\n');
		//applet.append('^FO320,20^BXN,10,100\n'); barcode pt 1
		//applet.append('^FD12345^FS\n');		barcode pt 2
		
		//second text was working but will now add wrapping  applet.append('^FO332,30^ADN,45,25^FDIRD-123\n^FS\n');
		applet.append('^FO332,30^FB60,2,,^ADN,50,27^FDIRD-1234567\n^FS\n');
		
		//applet.append('^FO80,50^ADN,36,20^FDWorld!\n^FS\n');
		applet.append('^XZ\n');	
		/*
		applet.append('^XA');	// start format...start job
		/****BARCODE*******
		//the position we want...commenting to try squeexintg stuff applet.append('^FO,20');
		 applet.append('^FO3O,30');
		applet.append('^BXN,1,200');
		//applet.append('^BXN,1,200,40,50,6,~');
		applet.append('^FD123456');//FD=field data
		//Does this need its own XZ to say end format???????????????????????
		applet.append('^FS\n');
		/****END BARCODE*****		
		/*******TEXT**********
		//applet.append('^FO20,20');
		//applet.append('^CFD^FDHiThere^FS');
		/******END TEXT************/				
		/**and another bar code it seems ***
		//does this need a FO Field origin?????????????????????????????????????
		//applet.append('^FO5O,30');
		//applet.append('^BXN,1,200,80,50,6,~');
		//applet.append('^FD124156');//FD=field data
		/***end another barcode *****/
		// applet.append('^FS\n');
		//applet.append('^XZ\n'); // end format - but brady doc says "end job" .. so perhaps it is per document...i think we have proven this 
		
		//applet.append('^XA\n^FO50,50^ADN,36,20^FDHello World!\n'); 		
		// applet.append('^FDHello World!\n');	
		// Send characters/raw commands to printer
		
		
		
		
		/*)alert('before print xml4');
		
		
		applet.append('<?xml version="1.0" ?>');
		applet.append('<bpl-document xmlns="http://www.bradycorp.com/printers/bpl">');
		applet.append('<defaults>');
			applet.append('<document units= "mm" />');
		applet.append('</defaults>');
		applet.append('<labels>');
		/*applet.append('	<label width= "10" height= "15.0">');
		applet.append('		<barcode position-x= "0.5" position-y= "1.0" height= "4.0" type= "datamatrix"');  //applet.append(' human-readable = "false" human-readable-location= "right" >');
		applet.append('			<datasource>');
		applet.append('				<static-text value= "Y123456" />');
		applet.append('			</datasource>');
		applet.append('		</barcode>');');
		*
		applet.append('	<label width= "15" height= "10.0">');
		applet.append('<text position-x="12	" position-y="-2" align="left" font-name="Arial" >');
		applet.append('	<text-sizing>');
		applet.append('		<autosize height="5.0" width="5.0" />');
		applet.append('	</text-sizing>');
		applet.append('	<datasource>');
		applet.append('		<static-text value="hi." />');
		applet.append('	</datasource>');
		applet.append('</text>');
		
		applet.append('		<barcode position-x= "0" position-y= "-2.0" height= "5.0" type= "datamatrix"');  //applet.append(' human-readable = "false" human-readable-location= "right" >');
		applet.append('			<datasource>');
		applet.append('				<static-text value= "Y123" />');
		applet.append('			</datasource>');
		applet.append('		</barcode>');
...
/*		applet.append('	<label width= "10" height= "15.0">');
		applet.append('		<barcode position-x= "7.5" position-y= "1.0" height= "4.0" type= "datamatrix"');  //applet.append(' human-readable = "false" human-readable-location= "right" >');
		applet.append('			<datasource>');
		applet.append('				<static-text value= "Y123" />');
		applet.append('			</datasource>');
		applet.append('		</barcode>');
		...
		applet.append('</labels>');
		applet.append('</bpl-document>');
		*/
		/* a random thing off a zebra page
		applet.append('^XA');
		applet.append('^FO20,0^BY3');
		applet.append('^B4N,20,A,A');
		applet.append('^FD12345ABCDE^FS');
		applet.append('^XZ');
		*/
		applet.print();
		// alert('after print');
...
function monitorApplet(appletFunction, finishedFunction, description) {
	var NOT_LOADED = "Program required for printing hasn't loaded yet.";
	var INVALID_FUNCTION = 'Invalid function: "' + appletFunction +'"';
	var INVALID_PRINTER = "Could not find a valid printer";
	if (document.jZebra != null) {
		var finished = false;
		try {
			finished = eval('document.jZebra.' + appletFunction);
		} catch (err) {
			alert('ERROR:  ' + INVALID_FUNCTION);
			return;
		}
		if (!finished) {
			...
			if (e != null) {
				...
				alert("ERROR: " + desc + document.jZebra.getExceptionMessage());

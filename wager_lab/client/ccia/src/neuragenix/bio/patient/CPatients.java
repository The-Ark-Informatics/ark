...
public class CPatients implements IChannel {
    ...
    /**
     *
     *  Contructs the CPatients Channel
     *
     */
    ...
    private String doViewPatient(){
        ...
        try{
            ...
            if( patientID != null ){
                ...
                if (rs1.next())
                {
                    // Convert the date for display from format yyyy-MM-dd to dd-MM-yyyy
                    SimpleDateFormat dfFormatter = new SimpleDateFormat("dd-MM-yyyy");
                    String strAdmissionDate = rs1.getString("ADMISSIONS_dtDiagnosis");
                    if (strAdmissionDate != null)
                    {    
                        java.sql.Date dtObject = java.sql.Date.valueOf(strAdmissionDate);
                        strAdmissionDate = dfFormatter.format(dtObject);
                    }
                    String strAdmissionCategory = rs1.getString("ADMISSIONS_strDiagCategory");

                    if (strAdmissionDate != null)
                    {    
                        strXML.append("<ADMISSIONS_dtDiagnosis>" + strAdmissionDate + "</ADMISSIONS_dtDiagnosis>");
                    }
    ...
    private String doAddPatient(){
        ...
        try{
            ...
            if ( runtimeData.getParameter( "save" ) != null )
            {
                ...
                //if ( runtimeData.getParameter( "PATIENT_strSex").equals("male") && 
                //((runtimeData.getParameter("PATIENT_strTitle").equals("Mrs")) || (runtimeData.getParameter( "PATIENT_strTitle").equals("Ms"))))
                //{
                //    strTitleError = "Please check that the title and sex fields match.";
                //}
                // Title Mr incorrect for female
                //if ( runtimeData.getParameter( "PATIENT_strSex").equals("female") && runtimeData.getParameter("PATIENT_strTitle").equals("Mr"))
                //{
                //    strTitleError = "Please check that the title and sex fields match.";
                //}
            ...
            if( patientID != null ){
                ...
                if( runtimeData.getParameter( "deleteAdmission" ) != null )
                {
                    ...
                    else
                    {
                        ...
                        query.setWhere( null, 0, "ADMISSIONS_intAdmissionkey", "=", 
                            runtimeData.getParameter( "ADMISSIONS_intAdmissionkey"), 0, DALQuery.WHERE_HAS_VALUE );
        ...
        try
        {
            ...
            if( ((strError = QueryChannel.checkRequiredFields( formfields, runtimeData )) == null ) && 
                ...
                if( lock.isValid() && lock.lockWrites() ){
                    ...
                    // Get the patient age at surgery
                    int intAge = getPatientAge(patientID, runtimeData.getParameter("ADMISSIONS_intAdmissionkey") );
                    query.setField ("ADMISSIONS_intPatientAge", Integer.toString(intAge));

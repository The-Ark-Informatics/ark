 ...
 */
	...
	<table width="100%">
		<tr>
			...
			Batch Processing - Available Locations<br/>
			</td>
		         
		            <td align="right">
		               <form name="back_form" action="{$baseActionURL}?module=batch_clone&amp;current=bp_clone&amp;stage=BACK" method="post">
	  ...
	  <xsl:for-each select="GenerationDetails">    
            ...
            <!-- check if the system has decided if there is available spaces or not -->
                <xsl:when test="GD_AvailableLocations/GD_NoAvailable='true'">
                   ...
                   Available : <xsl:value-of select="GD_AvailableLocations/GD_NoAvailable" />
                   <td>
                       <xsl:value-of select="SITE_strSiteName" />
                   </td>
                   <td>
                ...
                <xsl:otherwise>
                    <tr class="uportal-text">
                       ...
                       <xsl:choose>
                           <xsl:when test="(GD_AllocationMode=0 or GD_AllocationMode=1) and GD_AllocationStatus=-1">
                               <td><!--xsl:value-of select="SITE_strSiteName" /--><br />
                                   <form name="site-allocation" id="site-allocation" action="" method="">>
                           ...
                           <xsl:otherwise>
                           <td colspan="4"><span class="uportal-label"></span></td>
                           </xsl:otherwise>
                           </xsl:choose>
                       ...
                       <!-- debug code <xsl:value-of select="GD_AllocationMode" />:S-<xsl:value-of select="GD_AllocationStatus" />:N-<xsl:value-of select="string(GD_AllocationStatus)" /> -->
                       </td>
                       <td>
                          <xsl:choose>
                       ...
                       <td>
                          <xsl:if test="GD_AllocationStatus='-1'"> <!-- not yet allocated -->
                              <form name="DOALLOCATION_{GD_internalID}" id="DOALLOCATION_{GD_internalID}" action="{$baseActionURL}?module=batch_clone&amp;current=bp_clone&amp;stage=ALLOCATOR&amp;mode=DOALLOCATION&amp;intAllocationID={GD_internalID}" method="post">

                                  <input type="hidden" name="locationID" value="" />
                                  <xsl:choose>
                                     <xsl:when test="GD_AllocationMode=0 or GD_AllocationMode=1">
                                        
                                        <input type="button" onclick="javascript:submitAllocation({GD_internalID});" class="uportal-button" value="Allocate"></input>
                                     </xsl:when>
                                     <xsl:otherwise>
                                         <input type="button" onclick="javascript:document.DOALLOCATION_{GD_internalID}.submit()" class="uportal-button" value="Allocate"></input>
                                     </xsl:otherwise>
                                  </xsl:choose>
	...
	<br />
        ...
        <table width="100%">
            <tr>
               ...
               <xsl:if test="count(//GenerationDetails[GD_AllocationStatus=1100]) &gt; 0">
                <td align="left">
                   <form name="displayReport" action="{$baseActionURL}?module=batch_clone&amp;current=bp_clone&amp;stage=DISPLAYREPORT" method="post">
                      <input type="submit" value="Generate Report &gt;" class="uportal-button"/>
                   </form>
                </td>
                  </xsl:if>
            </tr>

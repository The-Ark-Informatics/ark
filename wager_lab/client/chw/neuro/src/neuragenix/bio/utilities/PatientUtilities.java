...
public class PatientUtilities
{
   ...
   // [SV] Function used by CInventory to get an element (such as first name) of a patient
   // Used in CInventory when displaying cell info of a tray
   public static String getPatientDetails (int patientID, String strElement, AuthToken authToken)
   {
        ...
        try
        {
            if (authToken.hasActivity("patient_view"))
            {
                ...
                if(rsResult.next())
                {
                    // For this to process correctly the strElement needs to be of form: "PATIENT_strPatientID"
                    strPatientID = rsResult.getString(strElement);
                }
            ...
            {
                System.out.println ("PatientUtilities::getStrPatientID] Warning no permissions for user " + authToken.getUserIdentifier() + ".");
            }
        }
        catch (Exception e)
        {

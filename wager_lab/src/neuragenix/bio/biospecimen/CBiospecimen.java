...
public class CBiospecimen implements IChannel
{
   ...
   public CBiospecimen()
   {
      ...
      try
      {
         pageSize = PropertiesManager.getPropertyAsInt("neuragenix.bio.RecordPerPage");
   ...
   public void receiveEvent(PortalEvent ev)
   {
      try
      {
         ...
         if((ev.getEventNumber() == ev.SESSION_DONE)||(ev.getEventNumber() == ev.UNSUBSCRIBE))
         {
   ...
   public void renderXML(ContentHandler out) throws PortalException
   {
      ...
      xslt.setXSL("CBiospecimens.ssl", rp.getStylesheet(), runtimeData.getBrowserInfo());
      
      // Specify the
      
      xslt.setStylesheetParameter("baseActionURL", runtimeData.getBaseActionURL());
      xslt.setStylesheetParameter("strErrorMessage", rp.getErrorMessage(true));
      xslt.setStylesheetParameter("lastViewedID", bcBiospecimen.getLastViewedBiospecimenID()+"");
      //System.out.println("ID is" + bcBiospecimen.getLastViewedBiospecimenID());
      ...
      upfTmp.setTargetNodeId(SessionManager.getChannelID(strSessionUniqueID, "CInventory"));
      xslt.setStylesheetParameter("inventoryChannelURL", upfTmp.getUPFile());
      xslt.setStylesheetParameter( "inventoryChannelTabOrder", SessionManager.getTabOrder( rp.getAuthToken(), "CInventory") );
      ...
      xslt.setStylesheetParameter( "patientChannelTabOrder", SessionManager.getTabOrder( rp.getAuthToken(), "CPatient") );
      ...
      xslt.setStylesheetParameter( "biospecimenChannelTabOrder", SessionManager.getTabOrder( rp.getAuthToken(), "CBiospecimen") );
   ...
   public void setRuntimeData(ChannelRuntimeData rd) throws PortalException
   {
     ...
     //Utilities.printRuntimeData(rd); 
      ...
      System.err.println("Biospecimen Channel : Current Module is - " + strModule);
      ...
      {
         if ((strBQCurrent.equals("build_isearch") || strBQCurrent.equals("isearch_result")) && runtimeData.getParameter("ISEARCH_finish") == null && mgrBatchQuantity != null)
         {
            rp.clearXML();
      ...
      if (runtimeData.getParameter("ISEARCH_finish") != null)
      {
        ...
        /*
         * -----------------------------------
         * End Batch Quantity Allocation code
         *
         */
      
      
        /*
         *  All new request handler
         *  Expects there to be a "module" parameter in everything
         *  otherwise it will default to search
         *
         */
      ...
      if (strModule == null)
      {
         ...
         if(lckBiospecimen != null)
         {
            lckBiospecimen.emptyAllLocks(rp.getAuthToken().getSessionUniqueID());
         }
        ...
        /* // show search screen
         rp.clearXML();
         if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_SEARCH, rp.getAuthToken()))
         {
            rp.addXML("<biospecimen>");
            rp.addXML(bcBiospecimen.getSearchCriteriaXML()
            + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null));
      ...
      else if (strModule.equalsIgnoreCase("CORE"))
      {
         ...
         else if (strAction.equalsIgnoreCase("view_biospecimen"))
         {
            ...
            try
            {
               intBiospecimenKey = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
            }
            catch (Exception ne)
            {
               ne.printStackTrace();
            ...
            if (intBiospecimenKey != -1)
            {
               ...
               try
               {
                  if (lckBiospecimen.isValid())
                     lckBiospecimen.unlock();
                  lckBiospecimen = new LockRequest(rp.getAuthToken());
                  lckBiospecimen.addLock("BIOSPECIMEN", intBiospecimenKey + "", LockRecord.READ_WRITE);
                  lckBiospecimen.lockDelayWrite();
                  blGotLock = true;
               }
               catch (Exception le)
               {
                  //System.out.println ("Warning : Unable to acquire read only lock on biospecimen " + intBiospecimenKey);
                  le.printStackTrace();
               ...
               if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_VIEW, rp.getAuthToken()) && rp.getAuthToken().hasRecordAccess(0, intBiospecimenKey))
               {
                  rp.addXML("<biospecimen>");
                  if (blGotLock)
                  {
                	  String bioXML = bcBiospecimen.buildViewBiospecimenXML(intBiospecimenKey, runtimeData);
                	  if(bioXML == null) {
                		  System.err.println("Biospecimen data returned is null");
//TODO: Something needs to be neater here - this is where ugly empty forms show up.
                	  }
                	  else 
                		  rp.addXML(bioXML);
                  }
                  else
                     rp.setErrorMessage("Unable to acquire a view lock on this specimen");
                  rp.addXML(FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null)
                  + "</biospecimen>");
               }
               else
               {
            	   System.err.println("Do not have permission for this activity. doh");
                  rp.setErrorMessage(ERROR_MESSAGE_PERMISSION_DENIED_BIOSPECIMEN);
               }
               ...
               if (subAction != null && subAction.compareToIgnoreCase("deleteVialCalculation")==0)
               {
            	   ...
            	   try
            	   {
            		   intBiospecimenToDelete = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
            	   }
            	   catch (Exception de)
            	   {
            		   de.printStackTrace();
            	   ...
            	   if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_DELETE, rp.getAuthToken()))
            	   {
            		   if (intBiospecimenToDelete != -1)
            		   {
            			   ...
            			   if (lckBiospecimen.isValid())
            			   {
            				   ...
            				   {
            					   strReturnValue = bcBiospecimen.deleteVialCalculation(intBiospecimenToDelete, runtimeData);
            				   }
            				   else
            				   {
            					   //System.out.println ("Unable to get a lock on this biospecimen");
            		   ...
            		   
            	   {
            		   ...
            		   rp.addXML("<biospecimen>"
            				   + bcBiospecimen.buildViewBiospecimenXML(intBiospecimenToDelete, runtimeData)
            				   + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null)
            				   + "</biospecimen>");
            ...
            if (intBiospecimenKey != -1)
            {
               try
               {
                  if (lckBiospecimen.isValid() == false)
                  {
                     ...
                     try
                     {
                        // System.out.println ("Adding a new lock on the refresh!");
                        lckBiospecimen.unlock();
                        lckBiospecimen = new LockRequest(rp.getAuthToken());
                        lckBiospecimen.addLock("BIOSPECIMEN", intBiospecimenKey + "", LockRecord.READ_WRITE);
                        lckBiospecimen.lockDelayWrite();
                     ...
                     {
                        //System.out.println ("There was an error renewing the lock");
                        le.printStackTrace();
                     }
                     
                  }
                  String strCurrentSubType = runtimeData.getParameter("BIOSPECIMEN_strSampleSubType");
                  bcBiospecimen.setIsRefresh(true);
                  //set it to null
                  runtimeData.setParameter("BIOSPECIMEN_strSampleSubType",(String) null);
                   
                	  	
                  rp.clearXML();
                  if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_VIEW, rp.getAuthToken()))
                  {
                     rp.addXML("<biospecimen>");
                     rp.addXML( bcBiospecimen.buildViewBiospecimenXML(intBiospecimenKey, runtimeData));
                     // add the sub type section accordingly
                    if ((runtimeData.getParameter("keepSubType") != null) && (!runtimeData.getParameter("keepSubType").equalsIgnoreCase("true")))
                     {
                            rp.addXML( QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSampleSubType", "", runtimeData.getParameter("BIOSPECIMEN_strSampleType")+""));

                            String strFirstSubType = QueryChannel.getFirstItemInLOVs("BIOSPECIMEN_strSampleSubType", runtimeData.getParameter("BIOSPECIMEN_strSampleType")+"");
                            if (BiospecimenCore.blUseSubTypeLR)
                            {
                                if (strFirstSubType != null)
                                {
                                    rp.addXML(QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSubTypeLR", "", strFirstSubType));
                                }
                            }
                     }
                     else 
                     {
                    	    rp.addXML( QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSampleSubType", strCurrentSubType, runtimeData.getParameter("BIOSPECIMEN_strSampleType")+""));
                            if (BiospecimenCore.blUseSubTypeLR)
                            {
                    	 	rp.addXML(QueryChannel.buildLOVXMLFromParent("BIOSPECIMEN_strSubTypeLR","", strCurrentSubType));
                            }
                     }
               ...
               {
                  System.err.println("[CPatients] Error when build refresh view");
                  en.printStackTrace();
               }
            }
         ...
         else if (strAction.equalsIgnoreCase("save_biospecimen"))
         {
            ...
            try
            {
               ...
               if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_ADD, rp.getAuthToken()))
               {
                  ...
                  try
                  {
                     ...
                     String strInternalBiospecimenID = runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID");
                     if (strInternalBiospecimenID != null)
                     {
                        
                        if (lckBiospecimen.lockWrites() == false)
                        {
                           //System.out.println ("[Biospecimen Save] - Unable to lock writes");
                           blLockSet = false;
                        }
                        
                     }
                     else {
                    	 value = runtimeData.getParameterValues("BIOSPECIMEN_strSampleType");
                    	 if ( value != null) {
                    	 System.err.println("GOT MULTI : " + value.length);
                    	 valueLength = value.length;
                    	 }
                    	 else valueLength = 0;
                    	 if (runtimeData.getParameter("BIOSPECIMEN_intMultiple") != null) {
                    	 Integer mult =  new Integer(runtimeData.getParameter("BIOSPECIMEN_intMultiple"));
                    	 if (mult != null) {
                     ...
                     if (blLockSet == true)
                     {
                    	 ...
                    	 String strEncounter = runtimeData.getParameter("BIOSPECIMEN_strEncounter");
                    	 if (strIntBiospecimenID == null) {
                    		 ...
                    		 {
                    			 if (multiple < 10) {
                    				 blAddedMulti = value.length* multiple;

                    				 for (int i=0; i < value.length; i++) {
                    					 runtimeData.setParameter("BIOSPECIMEN_strSampleType",value[i]);
                    					 for (int j=0; j< multiple; j++ ){

                    						 // System.err.println("** LAST SUFFIX: " + BiospecimenUtilities.getLastBioAddedSuffix( intPatientID,  strEncounter, strSampleType));

                    						 //if (j==0) //ie the first.	
                    						 strResult = bcBiospecimen.saveBiospecimen2(runtimeData);
                    						 //else
                  ...
                  catch (Exception le)
                  {
                     ...
                     //System.out.println ("There was an error doing the locking");
                     le.printStackTrace();
                  }
               }
               else
               ...
               if (strResult == null) // display the biospecimen
               {
                  ...
                  if (lckBiospecimen.isValid())
                     lckBiospecimen.unlock();
                  lckBiospecimen = new LockRequest(rp.getAuthToken());
                  lckBiospecimen.addLock("BIOSPECIMEN", bcBiospecimen.getLastBiospecimenAddedID() +"", LockRecord.READ_WRITE);
                  lckBiospecimen.lockDelayWrite();
                  
                  
                  
                  rp.addXML("<biospecimen>"
                  + bcBiospecimen.buildViewBiospecimenXML(bcBiospecimen.getLastBiospecimenAddedID(),runtimeData)
                  + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null)
                  + "</biospecimen>");
                  if (blAddedMulti > 1) {
                	  rp.setErrorMessage(blAddedMulti + " biospecimens saved");
                  }
                  else
               ...
               else
               {
                  ...
                  if (strCurrent.equalsIgnoreCase("biospecimen_view"))
                  {
                     ...
                     int intTempBiospecimenID = Integer.parseInt( runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
                     rp.addXML( bcBiospecimen.buildViewBiospecimenXML( intTempBiospecimenID , runtimeData)
                     + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null));
                     rp.setStylesheet(BiospecimenCore.ACTION_BIOSPECIMEN_VIEW);
                  ...
                  else
                  {
                     ...
                     rp.addXML(bcBiospecimen.buildAddBiospecimenXML(null, null, runtimeData));
                  }
                  rp.addXML("</biospecimen>");
         ...
         else if (strAction.equalsIgnoreCase("add_biospecimen"))
         {
            ...
            String strInternalPatientKey = runtimeData.getParameter("BIOSPECIMEN_intPatientID");
            String strInternalBiospecimenParentKey = runtimeData.getParameter("BIOSPECIMEN_intParentID");
            ...
            try
            {
               if (strInternalBiospecimenParentKey != null)
               {
                  ...
                  //lock the parent
                  lckBiospecimen.emptyAllLocks(rp.getAuthToken().getSessionUniqueID());
                  lckBiospecimen.addLock("BIOSPECIMEN", strInternalBiospecimenParentKey, LockRecord.READ_ONLY);
               ...
               if (strInternalPatientKey != null)
               {
                  ...
                  lckPatient.emptyAllLocks(rp.getAuthToken().getSessionUniqueID());
                  
                  lckPatient.addLock("PATIENT", strInternalPatientKey, LockRecord.READ_ONLY);
            ...
            //System.out.println("strInternalBiospecParent: " + strInternalBiospecimenParentKey);
            int intPatientKey = 0;
            ...
            {
               rp.addXML("<biospecimen>");
               rp.addXML(bcBiospecimen.buildAddBiospecimenXML(strInternalPatientKey, strInternalBiospecimenParentKey,runtimeData));
               rp.addXML("</biospecimen>");
            }
            else
            {
         ...
         else if (strAction.equalsIgnoreCase("delete_biospecimen"))
         {
            ...
            {
               intBiospecimenToDelete = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
               intParentKey = BiospecimenUtilities.getBiospecimenParentKey(intBiospecimenToDelete);               
            }
            catch (Exception de)
            {
               de.printStackTrace();
            ...
            if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_DELETE, rp.getAuthToken()))
            {
               if (intBiospecimenToDelete != -1)
               {
                  ...
                  if (lckBiospecimen.isValid())
                  {
                     ...
                     if (blLockAcquired == true)
                     {
                        strReturnValue = bcBiospecimen.deleteBiospecimen(intBiospecimenToDelete);
                     }
                     else
                     {
                        //System.out.println ("Unable to get a lock on this biospecimen");
            ...
            else
            {
                if (intParentKey != -1)
                {
                   ...
                   rp.addXML("<biospecimen>"
                   + bcBiospecimen.buildViewBiospecimenXML(intParentKey, runtimeData)
                   + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null)
                   + "</biospecimen>");
         ...
         else if (strAction.equalsIgnoreCase("save_quantity"))
         {
            ...
            try
            {
               int intBiospecimenKey = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
               ...
               if (intBiospecimenKey != -1)
               {
                  ...
                  int sitekey = InventoryUtilities.getSiteKeyforBiospecimen(new Integer(intBiospecimenKey).intValue());
         			...
         			if (!rp.getAuthToken().getSiteList().contains(ISitekey) && sitekey != -1 ) {
               ...
               rp.clearXML();
               rp.addXML("<biospecimen>"
               + bcBiospecimen.buildViewBiospecimenXML(intBiospecimenKey,runtimeData)
               + FlagManager.getFlaggedRecordsXML(FlagManager.DOMAIN_BIOSPECIMEN, null)
               + "</biospecimen>");
         ...
         else if (strAction.equalsIgnoreCase("delete_quantity"))
         {
            ...
            try
            {  intBiospecimenKey = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
               intBiospecimenTransactionID = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_TRANSACTIONS_intBioTransactionID"));
            }
            catch (Exception ne)
            {
               ne.printStackTrace();
            ...
            if (intBiospecimenTransactionID != -1 && intBiospecimenKey != -1 )
            {
               ...
               if (lckBiospecimen.isValid())
               {
                  ...
                  {
                     //System.out.println ("Unable to acquire a lock on the biospecimen");
                     le.printStackTrace();
                     blLockAcquired = false;
                  }
                  ...
                  else
                  {
                     if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_DELETE, rp.getAuthToken()))
                     {
                        strResult = bcBiospecimen.doDeleteTransaction(intBiospecimenKey, intBiospecimenTransactionID);
                     }
                     else
                     {
                        strResult = ERROR_MESSAGE_PERMISSION_DENIED;
                     ...
                     {
                        //System.out.println ("Unable to remove lock!");
                        le.printStackTrace();
                     }
         ...
         else if (strAction.equalsIgnoreCase("upgrade_quantity"))
         {
            ...
            {
               rp.setErrorMessage("Can not upgrade a transaction because transaction ID or biospecimenID is invalid");
               ne.printStackTrace();
            }
            if (intBiospecimenTransactionID != -1 && intBiospecimenKey != -1 )
            {
               ...
               if (lckBiospecimen.isValid())
               {
                  try
                  {
                     if (lckBiospecimen.lockWrites())
                     {
                        if (checkPermission(BiospecimenCore.PERMISSION_DELIVER_QUANTITY, rp.getAuthToken()))
                        {
                           strResult = bcBiospecimen.doTransactionUpgrade(intBiospecimenTransactionID);
                        }
                        else
                        {
                           strResult = ERROR_MESSAGE_PERMISSION_DENIED;
                  ...
                  catch (neuragenix.common.UnsupportedLockException le)
                  {
                     ...
                     //System.out.println ("Locking exception has occured");
                     le.printStackTrace();
                  }
         ...
         else if (strAction.equalsIgnoreCase("clone_biospecimen"))
         {
            ...
            // -- Shouldn't need to lock here, as we should already have a read lock on the biospecimen
            ...
            {
               System.err.println("[CBiospecimen] Can not parse Bio spec ID or study key");
               ne.printStackTrace();
            }
            ...
            {
               String strResult = null;
               if (checkPermission(BiospecimenCore.ACTION_BIOSPECIMEN_CLONE, rp.getAuthToken()))
               {
                  strResult = bcBiospecimen.cloneBiospecimen2( intBiospecimenKey,runtimeData);
               }
               else
               {
                  strResult = ERROR_MESSAGE_PERMISSION_DENIED;
               ...
               // it shouldnt attempt to load the biospecimen if the clone didnt occur
               ...
               if (strResult != null)
               {
                  rp.addXML(bcBiospecimen.buildViewBiospecimenXML(bcBiospecimen.getLastViewedBiospecimenID(), runtimeData));
               }
               else
               {
                  rp.addXML(bcBiospecimen.buildViewBiospecimenXML(bcBiospecimen.getLastBiospecimenAddedID(), runtimeData));
               }
            ...
            {
               System.err.println("Can not clone because Study Key or Biospecimen Key is invalid");
      ...
      {
//        if (checkPermission(BatchSFResultsManager.PERMISSION_BATCH_SFRESULTS, rp.getAuthToken()))
      ...
      else if (strModule.equalsIgnoreCase("ATTACHMENTS"))
      {
         ...
         if (strAction != null && strAction.equalsIgnoreCase("add_attachment"))
         {
            String strResult = null;
            
            if (checkPermission(AttachmentsAndNotes.PERMISSION_BIOSPECIMEN_ATTACHMENTS_ADD, rp.getAuthToken()))
            {
         ...
         else if (strAction != null && strAction.equalsIgnoreCase("delete_attachment") )
         {
            String strResult = null;
            
            if (checkPermission(AttachmentsAndNotes.PERMISSION_BIOSPECIMEN_ATTACHMENTS_DELETE, rp.getAuthToken()))
            {
         ...
         rp.clearXML();
         if (checkPermission(AttachmentsAndNotes.PERMISSION_BIOSPECIMEN_ATTACHMENTS_VIEW, rp.getAuthToken()))
         {
            rp.addXML("<biospecimenAttachments>" + attAttachments.buildViewAttachmentsXML(runtimeData)
            + bcBiospecimen.getBiospecimenDetailsXML(intBiospecimenKey,runtimeData) + "</biospecimenAttachments>");
         }
         else
         {
            rp.addXML("<biospecimenAttachments></biospecimenAttachments>");
      ...
      else if (strModule.equalsIgnoreCase("NOTES"))
      {
          ...
          if (strAction != null && strAction.equalsIgnoreCase("add_notes"))
          {
             ...
             if (checkPermission(AttachmentsAndNotes.PERMISSION_BIOSPECIMEN_NOTES_ADD, rp.getAuthToken()))
             {
          ...
          else if (strAction != null && strAction.equalsIgnoreCase("delete_notes") )
          {
             String strResult = null;
             
             if (checkPermission(AttachmentsAndNotes.PERMISSION_BIOSPECIMEN_NOTES_DELETE, rp.getAuthToken()))
             {
      ...
      else if (strModule.equalsIgnoreCase("BIOSPECIMEN_SEARCH"))
      {
         ...
         else if (strAction != null)
         {
            ...
            {
               int intExpandID = Integer.parseInt(runtimeData.getParameter("expandID"));
               if (htCurrentSearchCriteria != null) // not that it should be
               {
                  String strCurrentSearchPage = runtimeData.getParameter("PAGING_currentPage");
                  int intPageToDisplay = 0;
                  
                  if (strCurrentSearchPage == null)
                  {
                     // System.out.println("Was unable to retrieve the current page");
                     rp.setErrorMessage("Unable to retreive current page.  Can not move pages");
                  }
                  else
                  {
                  ...
                  if (intLastDomainSearch == BiospecimenCore.DOMAIN_STUDYALLOC_ALLOC)
                  {
                     rp.addXML(bcBiospecimen.getBiospecimenSearchResultsXML(intLastDomainSearch,  htCurrentSearchCriteria, false, intPageToDisplay, this.intCurrentPagingSize, false, intExpandID, true));   
                  }
                  else
                  {
            ...
            {
               // Called when coming from the patient channel into the biospecimen channel for doing things such as adding, etc
               ...
               String strInternalPatientKey = runtimeData.getParameter("intInternalPatientID");
               
               boolean hasCollection = true;
               boolean blCollectionTest = false;
               if (strInternalPatientKey != null)
               {
                  htCurrentSearchCriteria.put("BIOSPECIMEN_intPatientID", strInternalPatientKey);
                  
                  try
                  {
                      blCollectionTest = PropertiesManager.getPropertyAsBoolean("neuragenix.bio.Biospecimen.requireCollection");
                  }
                  catch (Exception e)
                  {
                      e.printStackTrace();
                  ...
                  hasCollection = PatientUtilities.checkForAdmissions(Integer.parseInt(strInternalPatientKey));
            ...
            else if (strAction.equals("studyalloc"))
            {
               ...
               if (runtimeData.getParameter("BIOSPECIMEN_strSampleType")!=null)
            	   htCurrentSearchCriteria.put("BIOSPECIMEN_strSampleType", runtimeData.getParameter("BIOSPECIMEN_strSampleType"));
               if (runtimeData.getParameter("BIOSPECIMEN_strSampleSubType") != null && ((String)runtimeData.getParameter("BIOSPECIMEN_strSampleSubType")).trim().equalsIgnoreCase("")!= true )
               {
            	   htCurrentSearchCriteria.put("BIOSPECIMEN_strSampleSubType", runtimeData.getParameter("BIOSPECIMEN_strSampleSubType"));
               }
               if (runtimeData.getParameter("BIOSPECIMEN_intStudyID")!=null)
            	   htCurrentSearchCriteria.put("BIOSPECIMEN_TRANSACTIONS_intStudyKey", runtimeData.getParameter("BIOSPECIMEN_intStudyID"));
               if (runtimeData.getParameter("BIOSPECIMEN_TRANSACTIONS_searchType")!=null)
                  htCurrentSearchCriteria.put("BIOSPECIMEN_TRANSACTIONS_searchType", runtimeData.getParameter("BIOSPECIMEN_TRANSACTIONS_searchType"));
            ...
            else if (strAction.equals("PAGING_setPagingSize"))
            {
               ...
               String strNewPagingSize = runtimeData.getParameter("PAGING_pagingSize");
               String strCurrentSearchPage = runtimeData.getParameter("PAGING_currentPage");
               ...
               {
                  intCurrentPagingSize = Integer.parseInt(strNewPagingSize);
               }
               else
               {
               ...
               intPageToDisplay = Integer.parseInt(strCurrentSearchPage);
               int intOldPagingSize =Integer.parseInt(runtimeData.getParameter("PAGING_intOldRecordPerPage"));
               int intOldTotalPages = Integer.parseInt(runtimeData.getParameter("PAGING_intOldTotalPages"));
               
               // this to avoid the new target page overpasses the total page
               // System.out.println(intPageToDisplay + "*" + intCurrentPagingSize+ "=" + intPageToDisplay * intCurrentPagingSize);
               // System.out.println(intOldPagingSize + "*" + intOldTotalPages + "=" + intOldPagingSize * intOldPagingSize );
               if (intPageToDisplay * intCurrentPagingSize > intOldPagingSize * intOldTotalPages);
               intPageToDisplay = 0;
            ...
            {
               String strNewSearchPage = runtimeData.getParameter("currentPage");
               
               int intPageToDisplay = 0;
               
               if (strNewSearchPage == null)
               {
                  rp.setErrorMessage("Unable to retreive new page.  Can not move pages");
               }
               else
               {
         ...
         else
         {
            ...
            for (int i = 0; i < vtSearchFields.size(); i++)
            {
               ...
               QueryChannel.updateDateValuesInRuntimeData(vtSearchFields, runtimeData);
               String rtValue = runtimeData.getParameter(fieldName);
               if (rtValue != null && (!rtValue.equals("")))
                  htCriteria.put(fieldName, rtValue.toUpperCase());               
            }
            ...
            rp.addXML("<biospecimen>");
            rp.addXML(bcBiospecimen.getBiospecimenSearchResultsXML(BiospecimenCore.DOMAIN_BIOSPECIMEN,  htCriteria, true, 0, this.intCurrentPagingSize, false, -1));
      ...
      else if (strModule.equalsIgnoreCase("BATCH_CLONE"))
      {
         ...
         if (checkPermission(BatchCloneManager.PERMISSION_BATCH_CLONE, rp.getAuthToken()))
         {
      ...
      {
         if (checkPermission(BatchQuantityManager.PERMISSION_BATCH_QUANTITY, rp.getAuthToken()))
         {
      ...
      {   String strAction = runtimeData.getParameter("action");
          int intBiospecimenID = Integer.parseInt(runtimeData.getParameter("BIOSPECIMEN_intBiospecimenID"));
          //allocate a new cell
             ...
             if (blLockSet == true )
             {
                ...
                if (strAction.equalsIgnoreCase("allocate"))
                {
                   ...
                   try
                   {
                      ...
                      int intCellID = Integer.parseInt(runtimeData.getParameter("CELL_intCellID"));
                      InventoryUtilities.saveBiospecimenLocation(intBiospecimenID, intCellID, rp.getAuthToken());
                      
                   }
                   catch (Exception ex)
                   {
                      System.err.println("Cant parse the CellID or BIOSPECIMEN ID in saving new cell");
                      ex.printStackTrace();
                   }
                }
                ...
                else if (strAction.equals("unallocate"))
                {
                   try
                   {
                      InventoryUtilities.unallocateBiospecimenFromTray(intBiospecimenID, rp.getAuthToken());
                   }
                   catch(Exception ex)
                   {
   ...
   public boolean checkPermission(String strActivity, AuthToken authToken)
   {
      ...
      {
         System.err.println ("[CBiospecimen] Error when checking permissions");
         e.printStackTrace();
         return false;
      }
   }
   ...
   public void setSearchResultsRuntimeProperties(int intCallingDomain, int intPageToDisplay, Hashtable htCriteria)
   {
   ...
   public void setViewBiospecimen(int intBiospecimenKey, ChannelRuntimeData runtimeData)
   {

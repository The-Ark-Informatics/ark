...
<xsl:include href="../biospecimen_menu.xsl"/>
  ...
  <xsl:param name="nextable"><xsl:value-of select="enoughBiospecimen" /></xsl:param>
...
function submitNext(nextable){
        if (nextable =="true")
        document.NEXT.submit();
        else alert("Please check that you have total biospecimen equal the amount required");
  ...
  <!-- 
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
		<tr>
			...
			Vial Calculation - Allocate sub-specimens
                        </td>
                        <td align="right" width="1%">
                           <table width="100%" cellpadding="0" cellspacing="0">
                              <tr align="right"><td>
                           <a href="{$baseActionURL}?module=vial_calculation&amp;action=back&amp;BIOSPECIMEN_intBiospecimenID={$intBiospecParentID}" method="post">
	...
	<table width="100%">
        ...
        <span class="uportal-label">Number to allocate :  </span> <span class="uportal-text"><xsl:value-of select="VIAL_CALCULATION_intNumberOfAllocatedBios" /></span><br />
                  ...
                  <!-- if allocation has already occured, do not show this -->
                     <span class="uportal-input-text">
                         <xsl:choose>
                            ...
                            <xsl:otherwise>
                               <a href="{$baseActionURL}?&amp;intAllocationID={GD_internalID}&amp;module=vial_calculation&amp;subaction=delete_option&amp;action=get_allocation_option">delete</a>
                            </xsl:otherwise>
                         </xsl:choose>
                     </span>
                  </td>
        ...
        <form name="addNewClone" action="{$baseActionURL}?module=vial_calculation" method="POST" class="uportal-label">
           <input type="hidden" name="action" value="get_allocation_option"/>
           ...
           <input type="hidden" name="subaction" value="add_option"/>
        ...
        <table>
           <tr>   
              <xsl:if test="$nextable='true'">
              <td>
                 
                 <form name="NEXT" action="{$baseActionURL}?module=vial_calculation&amp;action=confirm_allocation_option" method="POST">

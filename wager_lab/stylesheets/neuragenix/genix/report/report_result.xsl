...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="report">
        ...
        <xsl:param name="serverURL"><xsl:value-of select="serverURL"/></xsl:param>
        ...
        <xsl:if test="$query != ''">
            <iframe src="{$serverURL}/testpage.jsp?query={$query}&amp;strSessionID={$strSessionID}" height="600" frameborder="no" width="100%"></iframe>
        </xsl:if>

...
<!--
    Document    : icdcodes.xsl
    Created on  : September 20, 2004, 3:12 PM
    Author      : Daniel Muyrley
    Email       : dmurley@neuragenix.com
    Description : Displays a list of icd codes based on what has been loaded into
                  the database.  Serves as a picker for Smartforms
-->
...
<xsl:template match='icdcodes_picker'>
...
dTree.prototype.node = function(node, nodeId) {
	...
	var str = '&lt;div class="dTreeNode"&gt;';
        ...
        // if( a != b ){
            ...
            str += '&lt;input id="ICDCODES" type="checkbox" name="icdcodes_' + node.id + '" value="true" ';
            if( node.selected )
                str +='checked="true"';
...
dTree.prototype.select = function(id, status) {
        ...
        if( status ){
            ...
            if( (this.aNodes[n].pid != this.root.id) ){
                ...
                eval( 'document.icdcodesForm.icdcodes_' + this.aNodes[n].id + '.checked = true');
...
function submitSelected(){
    document.icdcodesForm.action = document.icdcodesForm.action + '&amp;icdcodesSelected=true';
    document.icdcodesForm.submit();
...
function submitSelected( currentField ){
    ...
    var icdCodesCount = parseInt( document.getElementById( 'icdCodesCount' ).getAttribute('value') ) ;
    
    var strValues = '';
    for( var i = 1; i &lt;= icdCodesCount; i++ ){

        var checkObj = eval( 'document.icdcodesForm.icdcodes_' + i );
        
        
        if( checkObj ){

            if( checkObj.checked ){
                strValues += document.getElementById( 'icdcodes_' + i ).getAttribute( 'nameValue' ) + "\n" ;
            }else{
    ...
    var currentObj = eval( "document.icdcodesForm." + currentField );
        ...
        <table width="100%">
			...
			Choose ICD Code<br/><hr/>
                ...
                <tr>
                    ...
                    <xsl:variable name="icdCodesCount"><xsl:value-of select="icdCodesCount"/></xsl:variable>
                    
                    <span id="icdCodesCount" value="{$icdCodesCount}"/>
                    <xsl:for-each select="icdcodes">
                        
                       
                    
                        <xsl:variable name="ICDCODES_intICDCodeKey"><xsl:value-of select="ICDCODES_intICDCodeKey"/></xsl:variable>
                        <xsl:variable name="ICDCODES_strICDCodeName"><xsl:value-of select="normalize-space(ICDCODES_strICDCodeName)"/></xsl:variable>
                        <xsl:variable name="ICDCODES_strICDCode"><xsl:value-of select="normalize-space(ICDCODES_strICDCode)"/></xsl:variable>
                        <xsl:variable name="ICDCODES_intParentKey"><xsl:value-of select="ICDCODES_intParentKey"/></xsl:variable>
                        
                    
                        <span id="icdcodes_{$ICDCODES_intICDCodeKey}" keyValue="{$ICDCODES_intICDCodeKey}"
                            nameValue="{$ICDCODES_strICDCode}: {$ICDCODES_strICDCodeName}" codeValue="{$ICDCODES_strICDCode}" 
                            parentValue="{$ICDCODES_intParentKey}">
                    ...
                    <form name="icdcodesForm" action="{$baseActionURL}?uP_root=root&amp;" method="post">
                    ...
                    -->
                        ...
                        for( var i = 1; i &lt;= icdCodesCount; i++ ){
                            ...
                            if( document.getElementById( 'icdcodes_' + i ) ){

                                d.add(document.getElementById( 'icdcodes_' + i ).getAttribute( 'keyValue') ,
                                document.getElementById( 'icdcodes_' + i ).getAttribute( 'parentValue'),
                                document.getElementById( 'icdcodes_' + i ).getAttribute( 'nameValue'),
                                document.getElementById( 'icdcodes_' + i ).getAttribute( 'codeValue'),
                                document.getElementById( 'icdcodes_' + i ).getAttribute( 'selectedValue'),'');
            ...
            <tr>
                ...
                <td align="right">
                   <form action="{$baseActionURL}?uP_root=root&amp;" method="POST">
                        ...
                        <input type="button"  name="icdcodesSelected" value="Submit" onclick="javascript:submitSelected('{$currentField}')" class="uportal-button" />
                    </form>
                </td>
            </tr>
        </table>

...
<xsl:template match='syndromes_picker'>
...
dTree.prototype.node = function(node, nodeId) {
	...
	var str = '&lt;div class="dTreeNode"&gt;';
        ...
        if( a != b ){
            ...
            str += '&lt;input id="SYNDROMES" type="checkbox" name="syndrome_' + node.id + '" value="true" ';
            if( node.selected )
                str +='checked="true"';
...
dTree.prototype.select = function(id, status) {
        ...
        if( status ){
            ...
            if( (this.aNodes[n].pid != this.root.id) ){
                ...
                eval( 'document.syndromeForm.syndrome_' + this.aNodes[n].id + '.checked = true');
...
function submitSelected(){
    document.syndromeForm.action = document.syndromeForm.action + '&amp;syndromeSelected=true';
    document.syndromeForm.submit();
...
function submitSelected( currentField ){
    ...
    var syndromeCount = parseInt( document.getElementById( 'syndromeCount' ).getAttribute('value') ) ;
    
    var strValues = '';
    for( var i = 1; i &lt;= syndromeCount; i++ ){

        var checkObj = eval( 'document.syndromeForm.syndrome_' + i );
        
        
        if( checkObj ){

            if( checkObj.checked ){
                strValues += document.getElementById( 'syndrome_' + i ).getAttribute( 'codeValue' ) + ": " +
                             document.getElementById( 'syndrome_' + i ).getAttribute( 'nameValue' ) + "\n" ;
            }else{
    ...
    var currentObj = eval( "document.syndromeForm." + currentField );
        ...
        <table width="100%">
			...
			Choose syndrome<br/><hr/>
                ...
                <tr>
                    ...
                    <xsl:variable name="syndromeCount"><xsl:value-of select="syndromeCount"/></xsl:variable>
                    
                    
                    <span id="syndromeCount" value="{$syndromeCount}"/>
                    <xsl:for-each select="syndrome">
                        
                        <xsl:variable name="SYNDROMES_intSyndromesKey"><xsl:value-of select="SYNDROMES_intSyndromesKey"/></xsl:variable>
                        <xsl:variable name="SYNDROMES_strSyndromesName"><xsl:value-of select="SYNDROMES_strSyndromesName"/></xsl:variable>
                        <xsl:variable name="SYNDROMES_strEPCCCode"><xsl:value-of select="SYNDROMES_strEPCCCode"/></xsl:variable>
                        <xsl:variable name="SYNDROMES_intParentKey"><xsl:value-of select="SYNDROMES_intParentKey"/></xsl:variable>
                        ...
                        <span id="syndrome_{$SYNDROMES_intSyndromesKey}" keyValue="{$SYNDROMES_intSyndromesKey}"
                            nameValue="{$SYNDROMES_strSyndromesName}" codeValue="{$SYNDROMES_strEPCCCode}" 
                            parentValue="{$SYNDROMES_intParentKey}">
                    ...
                    <form name="syndromeForm" action="{$baseActionURL}?uP_root=root" method="post">
                    ...
                    -->
                        ...
                        for( var i = 1; i &lt;= syndromeCount; i++ ){
                            ...
                            if( document.getElementById( 'syndrome_' + i ) ){

                                d.add(document.getElementById( 'syndrome_' + i ).getAttribute( 'keyValue') ,
                                document.getElementById( 'syndrome_' + i ).getAttribute( 'parentValue'),
                                document.getElementById( 'syndrome_' + i ).getAttribute( 'nameValue'),
                                document.getElementById( 'syndrome_' + i ).getAttribute( 'codeValue'),
                                document.getElementById( 'syndrome_' + i ).getAttribute( 'selectedValue'),'');
            ...
            <tr>
                ...
                <td align="right">
                   <form action="{$baseActionURL}?uP_root=root" method="POST">
                        ...
                        <input type="button"  name="syndromeSelected" value="Submit" onclick="javascript:submitSelected('{$currentField}')" class="uportal-button" />
                   </form>
                </td>
            </tr>
        </table>
